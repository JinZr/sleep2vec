...
from data_hires.metadata import (
    build_w_h_age_sex_center,
    extract_binary_labels,
    make_weighted_sampler_from_labels,
    process_metadata,
)
from data_hires.utils import filter_valid_sample_indices, load_npz
...
class DefaultDataset(BaseDataset):
    ...
    def __init__(
        ...
        if load_preset_path:
            ...
            exclude_for_validation = set(getattr(self, "length_validation_exclude_channels", []))
            validation_extractors = {k: v for k, v in extractors.items() if k not in exclude_for_validation}
            validation_tokenizers = {k: v for k, v in tokenizers.items() if k not in exclude_for_validation}
            validation_channel_names = None
            if channel_names is not None:
                validation_channel_names = [k for k in channel_names if k not in exclude_for_validation]
            validation_min_channels = min_channels
            if allow_missing_channels and validation_channel_names is not None:
                validation_min_channels = min(min_channels, len(validation_channel_names))
            if not validation_extractors:
                raise ValueError(
                    "No channels left for token-length validation after applying length_validation_exclude_channels."
                )
            self.data = filter_valid_sample_indices(
                ...
                channel_names=validation_channel_names,
                min_channels=validation_min_channels,
                tolerance=1,
            )
            if save_preset_path:
    ...
    def dataloader(self, device: str = "cpu") -> DataLoader:
        ...
        def collate_fn(indices, tolerance=1):
            ...
            # 1️⃣ 逐样本检查 token 长度，并裁剪输入通道到样本内最小长度。
            # stage5 is label-only in hires stage prediction; allow it to keep epoch-level length.
            stage5_tokens_per_label = max(1, int(getattr(self, "stage5_tokens_per_label", 10)))
            for sample in samples:
                has_feature_channels = any(k != "stage5" for k in sample.tokens.keys())
                reference_keys = [k for k in sample.tokens.keys() if k != "stage5"]
                if not reference_keys:
                    reference_keys = list(sample.tokens.keys())

                ref_lengths = [sample.tokens[k].shape[0] for k in reference_keys]
                max_len, min_len = max(ref_lengths), min(ref_lengths)
                if max_len - min_len > tolerance:
                    length_map = {k: int(v.shape[0]) for k, v in sample.tokens.items()}
                    raise ValueError(f"Token length mismatch > tolerance in sample {sample.id}: {length_map}")

                for k in reference_keys:
                    sample.tokens[k] = sample.tokens[k][:min_len]
                    sample.masks[k] = sample.masks[k][:min_len]

                if "stage5" in sample.tokens and has_feature_channels:
                    stage_len = min_len // stage5_tokens_per_label
                    stage_len = min(stage_len, int(sample.tokens["stage5"].shape[0]))
                    sample.tokens["stage5"] = sample.tokens["stage5"][:stage_len]
                    sample.masks["stage5"] = sample.masks["stage5"][:stage_len]

            def _sample_feature_len(sample: Sample) -> int:
                for key in sample.tokens.keys():
                    if key != "stage5":
                        return int(sample.tokens[key].shape[0])
                return int(next(iter(sample.tokens.values())).shape[0])

            batch = {
                "id": [s.id for s in samples],
                "length": torch.tensor(
                    [_sample_feature_len(s) for s in samples],
                    # device=device
                ),
        ...
        if allow_missing_channels and bucket_by_available_channels and sampler is None:
            from data_hires.samplers import AvailableChannelsBucketBatchSampler
