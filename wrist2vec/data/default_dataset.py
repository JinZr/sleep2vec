...
from wrist2vec.source_routing import normalize_channel_source_names, uses_explicit_channel_sources
...
class DefaultDataset(BaseDataset):
            ...
            if self._uses_explicit_channel_sources:
                ...
                if missing_payload:
                    raise ValueError(
                        ...
                        "Rebuild the wrist2vec preset with wrist2vec.preprocess.save_dataset_presets."
                    )
    ...
    def dataloader(self, device: str = "cpu") -> DataLoader:
        ...
        def collate_fn(indices, tolerance=1):
            ...
            for src in selected_sources:
                ...
                with load_npz(src.path) as npz:
                    ...
                    if isinstance(src_payload, dict) and isinstance(src_payload.get("channel_sources"), dict):
                        payload_channel_sources = {
                            key: list(value) for key, value in src_payload["channel_sources"].items()
                        }
                    else:
            ...
            if not expand_source_branches and len(chosen) == 2:
        ...
        if allow_missing_channels and self.is_train_set:
            ...
            from wrist2vec.data.samplers import PairFirstBatchSampler
        ...
        if allow_missing_channels and bucket_by_available_channels and sampler is None:
            from wrist2vec.data.samplers import AvailableChannelsBucketBatchSampler
