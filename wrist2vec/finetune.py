...
def supervised(args, config_bundle):
    ...
    exp_root = Path(f"log-wrist2vec-finetune/{args.version}/")
    ...
    logger = WandbLogger(
        project="wrist2vec-finetune",  # 相当于 TensorBoard 的 log dir
        name=f"w2v-finetune-{version}",  # run 名称
    ...
    try:
        ...
        checkpoint_callback = ModelCheckpoint(
            dirpath=f"log-wrist2vec-finetune/{version}/checkpoints",  # ← 你想要的目录
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fine-tune wrist2vec downstream models on PSG data.")
