...
from wrist2vec.preprocess.split_index_by_dataset import normalize_mask_frame
from wrist2vec.source_routing import normalize_channel_source_names
...
def _filter_index_df_for_required_channels(
    ...
    *,
    ...
    for channel in required:
        ...
        if explicit_source_names and not allow_missing_channels and len(available_mask_columns) != len(mask_columns):
        ...
        channel_mask = mask_frame.any(axis=1) if allow_missing_channels else mask_frame.all(axis=1)
    ...
    if allow_missing_channels:
        ...
        if effective_min_channels < 1:
            raise ValueError("min_channels must be >= 1.")
        # Only channels with mask columns participate in CSV prefilter counting.
        threshold = min(effective_min_channels, len(channel_masks))
        mask_count = pd.concat(channel_masks, axis=1).sum(axis=1)
        filtered = df.loc[mask_count >= threshold].copy()
    else:
...
def main() -> None:
    ...
    expand_source_branches = isinstance(config_data.get("finetune"), dict)
    if args.allow_missing_channels is None:
        allow_missing_channels = not expand_source_branches
    else:
