...
def _resolve_adapt_run_artifacts(
    *,
    ...
    return AdaptRunArtifacts(
        save_path=Path("log-wrist2vec-adapt") / run_name / "checkpoints",
        run_name=run_name,
        wandb_id=None,
        trainer_ckpt_path=None,
...
def wrist2vec_adapt(args):
    ...
    logger = WandbLogger(
        project="wrist2vec-adapt",
        name=f"w2v-adapt-{run_name}",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run staged modality adaptation for wrist2vec.")
