...
## Required inputs
Requires a `sleep2vec`, `sleep2vec2`, or `sleep2expert` variant; RoFormer model config with `model.cls.embedding_type: bert`; checkpoint; explicit NPZ index CSV; eval split; unique model channels; fresh absolute embedding directory; and the fixed whole-night contract (`both`, final layer, NPZ output, batch size 1).
...
## Validation gates
Planning strictly loads the selected pretrain or finetune config, validates the selected index rows and token cap, checks every referenced NPZ file, and rejects non-empty, plan-overlapping, or experiment-managed `plans`/`reports`/`steps` output directories. Finetune configs must not own a preset, their data channels must match model channels, and every selected index row must satisfy any config-owned dataset-source filter. When `source` is absent or blank, filtering uses the authored index path, matching runtime behavior. Inspect the frozen config and command before launch.
