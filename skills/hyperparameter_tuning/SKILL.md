...
All search sources must pass canonical variant validation after their complete YAML overrides; registration and launch dry-run check the frozen candidate bytes. Read the preflight card as separate planner-local config and target CLI evidence, not proof of model construction, checkpoint compatibility, forward/backward, or GPU execution. The checks reuse identical config bytes only within a call and do not perform full sidecar-table reads per candidate.
