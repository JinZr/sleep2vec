...
| Variant embedding extraction | `sleep2vec2.extract_embeddings` | sleep2vec2 variant maintainer |
...
## sleep2vec2 Embedding Extraction

The package-local `sleep2vec2.extract_embeddings` entrypoint builds and strictly
loads the complete model config while allowing an explicit model-channel subset
for data loading and export. Whole-night extraction is an NPZ-only, one-sample-
per-path mode with a caller-supplied hard source-token cap; it rejects clipping,
...
position capacity. The extractor does not own subject pooling, downstream model
selection, or sealed-test authorization.
