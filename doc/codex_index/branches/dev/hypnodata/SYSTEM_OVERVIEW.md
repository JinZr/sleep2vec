...
7. `hypnodata.preprocess.truncate_to_common` aligns available raw signals to a
   common duration.
8. Adapter annotations may materialize configured `stage5`.
...
`preprocess_steps`. Structured filter/notch steps must be recorded as real
executed steps, never as `not_implemented` placeholders.
