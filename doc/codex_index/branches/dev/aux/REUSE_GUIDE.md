...
| Metadata auxiliary pooling | `sleep2vec.downstreams.temporal_aggregation.build_temporal_aggregator` plus the pooled-feature reuse inside `Sleep2vecDownstreamModel.forward` | Keeps metadata auxiliary tasks on the same shared sequence features instead of opening a second encoder path | Ad hoc sequence pooling inside the trainer |
| Metadata auxiliary prediction | `sleep2vec.downstreams.head_registry.create_head` with the existing `classification` / `regression` heads | Reuses the existing downstream head family for pooled metadata prediction without introducing a parallel auxiliary-head registry | A second auxiliary-only head registry |
...
- For metadata auxiliary tasks, reuse the existing main head registry with pooled features before inventing new head families.
...
8. Metadata auxiliary tasks should reuse the existing `classification` / `regression` heads and temporal aggregators; a second auxiliary-head registry would be parallel infrastructure.
