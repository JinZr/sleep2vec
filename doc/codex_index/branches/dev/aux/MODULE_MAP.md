...
| Finetune CLI binding | `sleep2vec/common.py` | Copy finetune YAML state into CLI namespace, enforce built-in task rules, and derive seq-label runtime flags | `sleep2vec.config` | New built-in tasks, CLI serialization | Do not parse finetune YAML directly in entrypoints |
...
| Runtime utilities | `sleep2vec/checkpoints.py`, `sleep2vec/metrics.py`, `sleep2vec/results.py`, `sleep2vec/distributed.py`, `sleep2vec/diagnostics.py`, `sleep2vec/callbacks/`, `sleep2vec/visualization/` | Checkpoint averaging, metrics, result persistence, process-rank helpers, torch.distributed readiness/rank-size helpers, diagnostics hooks, runtime callbacks, visualization artifacts | Torch, sklearn, wandb, matplotlib | New logging/export surfaces | `ahi` pointwise-train metrics and final event-based evaluation stay in `metrics.py`; result CSV writes live in `results.py`; env-based rank checks plus torch.distributed readiness/rank-size helpers live in `distributed.py` |
