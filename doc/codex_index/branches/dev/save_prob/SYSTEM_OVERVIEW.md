...
6. Bind automatic output paths under `results/inference/{namespace}/{label_name}/{prediction_run_id}`.
7. Run `trainer.test(...)`.
8. Write run-local metrics, run-local predictions, global `overview.csv`, and `run_manifest.json`.
...
- Finetune results table: caller-specified CSV via `sleep2vec.results.save_result_csv`
- Inference results: automatic `results/inference` run directories with run-local metrics, run-local predictions, global `overview.csv`, and `run_manifest.json`
