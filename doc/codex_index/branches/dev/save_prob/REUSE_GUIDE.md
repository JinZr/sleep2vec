...
| Inference result organization | Package-local `results.prepare_inference_result_paths` plus `save_result_csv`, `save_prediction_csv`, and `save_inference_manifest` | Creates the automatic `results/inference` run directory, global overview row, shared `prediction_run_id`, checkpoint tag, and manifest contract | Entry-point-local path construction or manually paired metrics/prediction files |
| Result CSV output | `sleep2vec.results.save_result_csv` | Preserves rank-zero gating, lockfile semantics, schema expansion, standard metadata columns, and automatic inference run metadata for aggregate metrics and overview rows | One-off CSV writers or the removed `metrics.save_result_csv` path |
| Inference prediction CSV output | Package-local `sleep2vec_inference.build_prediction_rows` / `build_ahi_prediction_rows` plus `results.save_prediction_csv` | Preserves the aggregate/detail join through `prediction_run_id`, keeps per-path export aligned with task-specific prediction semantics, and respects variant namespace boundaries | Standalone inference scripts that recompute labels, probabilities, or AHI summaries |
...
- Reuse `persist_run_config_and_args`, `prepare_inference_result_paths`, `save_result_csv`, `save_prediction_csv`, `save_inference_manifest`, package-local `sleep2vec_inference` row builders, and checkpoint helpers instead of duplicating serialization and output logic.
