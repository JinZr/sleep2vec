...
- Purpose and contract: run forward, compute loss if valid labels exist, add sleep2expert downstream MoE regularization/metrics when configured in that namespace, log stage loss or accumulate eval loss, cache either generic predictions or AHI event records for epoch-end metrics, and collect test-only prediction export records when enabled.
- Important inputs/outputs: batch and stage name in; returns training loss for `train`, otherwise `None`.
- Side effects: logs Lightning metrics and mutates `_stage_outputs` plus `_prediction_records` for detailed inference export.
- Key callers/callees: callers are `training_step`, `validation_step`, and `test_step`; callees are `self.model(...)`, `_compute_loss`, `_extract_valid_predictions`, package-local `extract_prediction_records`, and `_extract_ahi_event_records`.
...
## `sleep2vec.sleep2vec_inference.build_prediction_rows` and `build_ahi_prediction_rows`
...
- Purpose and contract: aggregate test-stage prediction records to one CSV-ready row per source path, averaging scalar windows and concatenating sequence windows by `token_start`.
- Important inputs/outputs: raw prediction records in; row mappings with path, ground truth, prediction, probabilities/scores, counts, and token starts out.
- Side effects: none.
- Key callers/callees: caller is `_finalize_epoch`; AHI rows reuse `_merge_ahi_window_records` and `_evaluate_single_ahi_record`.
- Reuse guidance: use these package-local helpers for inference prediction CSV rows after `results.prepare_inference_result_paths` has bound the automatic run-local prediction path.
- Duplication risk notes: do not regroup paths or recompute stage/AHI labels in `infer.py` or external scripts.

## `Sleep2vecFinetuning._finalize_epoch`

- File: `sleep2vec/sleep2vec_finetuning.py`
- Signature: `Sleep2vecFinetuning._finalize_epoch(stage: str)`
- Purpose and contract: reduce cached outputs for one stage, log generic or AHI-specific metrics, emit eval visualizations, populate `prediction_rows` for test-stage prediction export, and clear the stage cache.
...
- Side effects: logs metrics, writes W&B visualizations, updates the fitted AHI threshold, populates `prediction_rows`, and clears cached outputs.
- Key callers/callees: callers are `on_train_epoch_end`, `on_validation_epoch_end`, and `on_test_epoch_end`; callees include `_compute_reduced_ahi_train_pointwise_metrics`, `_compute_or_broadcast_ahi_metrics`, `compute_downstream_metrics`, package-local prediction row builders, and `DownstreamEvalVisualizer`.
