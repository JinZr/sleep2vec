...
On commit `4a80f9bf40ac7e8cb00143b5dc5b5eb5b15710dd`, tracked variant coverage is:

- `sleep2vec2/`: active standalone dense mirror with 102 tracked files
- `sleep2expert/`: active standalone MoE-capable mirror with 106 tracked files
...
- Important inputs/outputs: same pretrain/adapt/finetune/infer contracts as root `sleep2vec`, including package-local finetune imbalance loss/sampler schema, distributed-aware weighted metadata sampler, automatic inference result organization, prediction CSV export, package-local Kaldi conversion, and dataset routing.
...
- Important inputs/outputs: same dense runtime inputs as root for non-MoE recipes, including package-local finetune imbalance loss/sampler schema, distributed-aware weighted metadata sampler, automatic inference result organization, and prediction CSV export; MoE recipes add `model.backbone.moe` and optional `finetune.moe_tuning` blocks.
...
- `sleep2expert.config._build_finetune_moe_tuning_config`: MoE finetune mode, LR-scale, and regularization parser.
...
- `sleep2vec2.results.prepare_inference_result_paths` / `sleep2expert.results.prepare_inference_result_paths`: package-local automatic inference result directory, overview, manifest, and checkpoint-tag binding.
- `sleep2vec2.sleep2vec_inference.*` and `sleep2expert.sleep2vec_inference.*`: package-local prediction CSV row extraction and aggregation for all finetune recipe task families.

## Ownership Notes

- `sleep2vec2/` is a dense standalone variant boundary. Keep it package-local and parity-tested.
- `sleep2expert/` is the active MoE standalone variant boundary. Keep MoE schema, routing, regularization, tuning, and export changes inside this namespace unless deliberately changing root contracts.
- Automatic inference result organization and prediction CSV behavior are shared contracts but not shared imports: update root and variant package-local `infer.py`, `results.py`, `sleep2vec_finetuning.py`, and `sleep2vec_inference.py` together.
