...
- Purpose and contract: canonical inference driver; normalizes config, builds trainer and loader, optionally averages checkpoints, creates automatic inference result paths and run IDs, runs evaluation, writes run-local aggregate metrics, appends the global overview row, writes per-path prediction rows, and writes the run manifest.
- Important inputs/outputs: namespace in; no direct return value.
- Side effects: optional W&B run, trainer evaluation, automatic `results/inference` CSV and manifest output.
...
- Reuse guidance: extend package-local `infer.py` for inference-only orchestration changes.
- Duplication risk notes: checkpoint averaging policy belongs here plus `checkpoints.py`; automatic result path/run-id binding belongs in package-local `results.py`, not in standalone scripts.
...
- Purpose and contract: append a metrics row plus selected run metadata to a CSV file, including automatic inference run metadata when present, using rank-zero gating and a lockfile to tolerate concurrent local writers.
...
- Reuse guidance: reuse for run-local aggregate metrics and global overview rows.
- Duplication risk notes: current column ordering and schema-expansion policy is encoded here; do not use it for per-path prediction rows.

## `sleep2vec.results.prepare_inference_result_paths`

- File: `sleep2vec/results.py`
- Signature: `prepare_inference_result_paths(args, *, namespace: str = "sleep2vec", root: str | Path = DEFAULT_INFERENCE_RESULTS_ROOT, checkpoint_paths: Sequence[str | Path] | None = None, timestamp: str | None = None) -> None`
- Purpose and contract: bind automatic inference output paths and run metadata onto `args`.
- Important inputs/outputs: runtime args, namespace, optional checkpoint-averaging paths, and optional timestamp in; mutates `args` with `prediction_run_id`, `run_dir`, `inference_metrics_csv_path`, `inference_prediction_csv_path`, `inference_overview_csv_path`, `manifest_path`, checkpoint identity fields, and `task_family`.
- Side effects: none on disk.
- Key callers/callees: caller is package-local `infer.run_inference`; callees include `_resolve_checkpoint_info`, `_resolve_task_family`, and `make_prediction_run_id`.
- Reuse guidance: call this before `trainer.test(...)` so detailed prediction export is enabled and run-local output paths are available.
- Duplication risk notes: do not rebuild run-directory names or checkpoint tags in `infer.py`.
...
- Purpose and contract: append per-path inference prediction rows to a run-local CSV file with the same run metadata and `prediction_run_id` used by aggregate and overview rows.
- Important inputs/outputs: row mappings and destination path in; no return value.
- Side effects: creates directories, locks a sibling file, serializes list-like values as JSON strings, and writes or appends to CSV.
- Key callers/callees: caller is `infer.run_inference`; callees include `_resolve_result_source`, `_resolve_experiment_version`, `_ordered_prediction_columns`, `_result_csv_lock`, and `is_rank_zero_process`.
- Reuse guidance: use for detailed inference prediction export instead of extending `save_result_csv` beyond its one-row metrics contract.
- Duplication risk notes: keep rank-zero gating and append/schema-expansion behavior aligned with `save_result_csv`.

## `sleep2vec.results.save_inference_manifest`

- File: `sleep2vec/results.py`
- Signature: `save_inference_manifest(args, metrics: Mapping[str, Any] | None = None, *, prediction_row_count: int = 0) -> None`
- Purpose and contract: write the self-contained `run_manifest.json` for one inference run.
- Important inputs/outputs: runtime args, aggregate metrics, and prediction-row count in; JSON manifest out.
- Side effects: rank-zero-only atomic JSON write under the automatic run directory.
- Key callers/callees: caller is `infer.run_inference`; callee is `_write_json_atomic`.
- Reuse guidance: use alongside `save_result_csv` and `save_prediction_csv` for automatic inference outputs.
- Duplication risk notes: manifest content should come from the same args metadata populated by `prepare_inference_result_paths`.

## `sleep2vec.sleep2vec_inference.*`

- File: `sleep2vec/sleep2vec_inference.py`
- Signatures:
  - `prediction_export_enabled(args) -> bool`
  - `extract_prediction_records(args, batch, logits, targets) -> list[dict[str, object]]`
  - `build_prediction_rows(records: list[dict[str, object]]) -> list[dict[str, object]]`
  - `build_ahi_prediction_rows(records: list[dict[str, np.ndarray]], threshold: float) -> list[dict[str, object]]`
- Purpose and contract: convert test-stage logits/targets plus AHI event records into CSV-ready per-path prediction rows for scalar classification, scalar regression, stage sequence classification, and AHI/multilabel sequence recipes.
- Important inputs/outputs: batch metadata paths, token starts, logits, already-remapped targets, and AHI threshold in; row dictionaries with path, groundtruth, prediction, probabilities, counts, token starts, and AHI summary fields out.
- Side effects: none.
- Key callers/callees: callers are package-local `Sleep2vecFinetuning._shared_step` and `_finalize_epoch`; AHI row building calls package-local metrics helpers.
- Reuse guidance: use this module for prediction-export row construction instead of adding methods back to the Lightning module or recomputing labels in entrypoints.
- Duplication risk notes: `sleep2vec2` and `sleep2expert` intentionally carry package-local copies to preserve namespace guards.
