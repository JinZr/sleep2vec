...
- Side effects: opens sorted Kaldi scp readers on first access and resets them after process forks.
...
- Important inputs/outputs: for `backend="npz"`, exactly one of `preset_path` or `index`; for direct `backend="kaldi"`, `kaldi_data_root`, `kaldi_manifest`, and one split; for grouped Kaldi generation, `preset_path` may supply Kaldi `SampleIndex` rows while specs still come from the split-aware Kaldi manifest; batch samples with `clean_signals`, `observed_signals`, `availability_mask`, `quality_mask`, `corruption_mask`, `epoch_index`, `night_position`, `metadata`, and task modality fields out.
...
- Important inputs/outputs: sleep2wave config and index CSVs in; `manifest.json` with `format_version: 2` out; per-split CSVs under `manifests/`; one sorted ark/scp pair per canonical modality under `channels/<split>/`.
