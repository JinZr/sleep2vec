...
| Finetune data loaders | `sleep2vec.utils.get_finetune_dataloaders` and `_build_finetune_loader` | Own split/source choices and built-in sequence pseudo-channel behavior (`stage5`, `ahi`) including `stage5` as an auxiliary eval channel for `ahi` | Hand-rolled finetune loader creation |
...
- Keep raw `ahi` handling separate from sleep-stage remaps; `ahi` consumes raw `ahi` tokens plus auxiliary raw `stage5`.
