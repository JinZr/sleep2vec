...
- Purpose and contract: canonical finetune orchestration routine; persists run artifacts, builds loaders, instantiates `Sleep2vecFinetuning`, trains, evaluates, and writes results. When running distributed built-in `ahi` finetune, it keeps Lightning's progress bar enabled but inserts an epoch-end barrier callback immediately after the progress-bar callback so rank-zero-only progress updates cannot delay later epoch-end hooks relative to the other ranks.
