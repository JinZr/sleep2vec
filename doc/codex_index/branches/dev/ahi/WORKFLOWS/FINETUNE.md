...
- `ahi` validation/test checkpoint selection uses event-based AHI metrics with split post-processing: validation fits `ahi_eval_threshold` from cached probabilities on a coarse `0.1..0.9` grid, runs that search only on global zero, and broadcasts the resulting `{metrics, threshold}` to the other ranks. The training-time test stage reuses the persisted threshold by default. Separate `sleep2vec.infer` runs can search the cached inference probabilities on the fine `0.01..0.99` grid without rerunning the model, and fall back to the saved checkpoint threshold when no `TST >= 2h` summary sample exists. Detection stats continue to use merged + duration-filtered events, scalar NPZ `ahi` remains the summary ground truth, and scalar summary AHI is counted from stage-filtered raw predicted positive runs without merge or min-duration filtering so it aligns with NPZ `ahi`. Scalar NPZ `tst` remains the summary denominator, exact duplicate gathered windows are ignored before contiguity checks, and `TST < 2h` exclusion still applies to final summary metrics.
