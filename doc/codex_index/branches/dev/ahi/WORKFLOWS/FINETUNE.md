...
- `ahi` validation/test checkpoint selection uses event-based AHI metrics with split post-processing. Full validation (`val_ahi_pearson` + `max`) still fits `ahi_eval_threshold` from cached probabilities on a coarse `0.1..0.9` grid, runs that search only on global zero, and broadcasts the resulting `{metrics, threshold}` to the other ranks. Validation/test loss is accumulated locally during step execution and then reduced once at epoch end before logging, rather than relying on Lightning's eval-side `sync_dist=True` step logging. Lightweight validation skips event-record extraction, threshold fitting, Pearson, and summary scatter, and logs only emitted pointwise AHI metrics beside that manually reduced `val_loss`. After the rank-zero-only AHI summary scatter finishes, all ranks synchronize through the strategy barrier before leaving the validation/test epoch, so train-epoch metric collectives cannot run ahead of the visualization path. Train AHI pointwise metrics are computed from globally reduced confusion counts (accuracy/precision/recall/F1 only; no train ROC-AUC), avoiding epoch-wide token concatenation. When finetune used lightweight validation, the training-time automatic test and `--epochs 0` finetune evaluation inject a coarse test search grid so the existing test event-eval path can still run without a saved validation-fitted threshold. Separate `sleep2vec.infer` runs remain on the fine `0.01..0.99` grid without rerunning the model.
- Multi-device built-in `ahi` finetune installs an explicit custom progress-bar callback that preserves batch-level updates but skips the default rank-zero-only train-epoch-end UI refresh/postfix work.
