...
- `ahi`: seq multi-label classification, `output_dim=30`, raw labels from `ahi`, monitor `val_f1`
...
- `ahi` reuses the normal sequence head path but switches trainer loss/metrics to BCE plus flattened per-second binary reduction.
