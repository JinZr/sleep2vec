...
| Dataset core | `data/default_dataset.py`, `data/psg_pretrain_dataset.py`, `data/kaldi_io.py`, `data/kaldi_psg_dataset.py` | Build `SampleIndex` windows, validate/load presets, materialize NPZ or Kaldi batches, backfill built-in AHI metadata, and choose samplers | `data.utils`, `data.metadata`, `data.samplers`, optional `kaldi_native_io` | New dataset formats or collate behavior | Main data contract boundary; standalone variants must keep package-local mirrors |
...
| Preprocessing CLIs | `preprocess/*.py` | CSV splitting, preset generation/merge, required-channel prefiltering, missing-mask stats, WatchPAT conversion, NPZ-to-Kaldi conversion | pandas, pickle, NumPy, optional EDF deps, optional `kaldi_native_io` | New data-prep utilities | CLI tools are the canonical prep surface |
...
- `DefaultDataset.dataloader` -> `data.utils` + `data.metadata` + `data.samplers`; Kaldi subclasses override source availability/token loading hooks while preserving the batch contract
...
- Kaldi backend edits usually span `data/kaldi_io.py`, `data/kaldi_psg_dataset.py`, `preprocess/convert_npz_to_kaldi.py`, `sleep2vec/common.py`, `sleep2vec/utils.py`, and matching package-local variant mirrors.
