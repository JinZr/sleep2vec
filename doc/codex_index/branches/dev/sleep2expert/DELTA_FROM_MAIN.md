...
- `sleep2vec2` keeps the same YAML schema, CLI shape, dataloader behavior, preset shape, and model class names as the base recipe, including package-local Kaldi backend support.
...
- Kaldi-specific `sleep2vec2` runtime tests require `kaldi_native_io`; they should skip when that optional dependency is unavailable.
