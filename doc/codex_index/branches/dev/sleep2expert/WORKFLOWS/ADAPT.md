...
3. Bind model-derived fields into `args`.
   ...
   - `channel_input_dims`
   - `backbone_arch`
   - `data_backend`, `kaldi_data_root`, and `kaldi_manifest` when Kaldi is selected
   - initial `train_pair_probs`
4. Build loaders.
   - Reuses the missing-channel pretrain dataloader path.
   - Uses `KaldiPSGDataset` when the pretrain-style YAML selects `data.backend: kaldi`.
...
- Kaldi adaptation uses manifest-backed data and rejects legacy NPZ preset pickles.
