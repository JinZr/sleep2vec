...
| Sample validation | `data.utils.filter_valid_sample_indices` | Produces `payload["available_channels"]`, persists built-in AHI scalars, drops broken or all-ignored built-in AHI samples early, and uses automatic thread selection when callers leave `max_workers=None` | Custom preset-building loops |
