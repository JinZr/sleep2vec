...
7. Test.
   ...
   - The W&B run created by this finetune call is finished explicitly after result export so interpreter shutdown does not fall back to the W&B `atexit` teardown path; if training or evaluation already failed, teardown failures are warning-only so the primary error is preserved.
