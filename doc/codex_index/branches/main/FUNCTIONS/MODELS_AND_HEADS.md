...
- Purpose and contract: gather distributed survival records, load disease names only for sidecar-backed runs, skip disease-column sidecar parsing for preset-backed runs, optionally build test prediction rows, aggregate to subject-level risk rows, retain per-disease metric rows, log validation/test Cox loss, and log mean disease c-index when finite.
