...
- Purpose and contract: reduce per-sample predictions into classification or regression metrics, with stage-specific metrics for 5-class stage prediction.
- Important inputs/outputs: ground truth and predictions in, metrics dict out.
- Side effects: none.
- Key callers/callees: caller is `Sleep2vecFinetuning._finalize_epoch`; callee is `roc_auc_from_two_logits` for binary classification.
