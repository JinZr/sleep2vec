...
- Purpose and contract: Populate `args.output_dim`, `args.is_classification`, `args.is_seq`, `args.monitor`, and `args.monitor_mod` from either built-in label semantics or explicit finetune task config.
- Important inputs/outputs: mutates `argparse.Namespace` in place.
- Side effects: namespace mutation only.
- Key callers/callees: caller is `apply_finetune_config`; callees are `_validate_builtin_task_cfg` and `_validate_metadata_label_support`.
- Reuse guidance: use for any finetune/infer code path that depends on label semantics.
- Duplication risk notes: built-in task semantics must remain centralized here.
