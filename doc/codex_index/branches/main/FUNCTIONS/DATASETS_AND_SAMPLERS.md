...
- Duplication risk notes: the built-in label-channel registry and CSV-to-`SampleIndex` survival/multilabel sidecar attachment live here and should not be replicated in caller code.
...
## `data.multilabel.load_multilabel_label_table`

- File: `data/multilabel.py`
- Signature: `load_multilabel_label_table(config: Any | None, expected_output_dim: int | None = None) -> MultilabelLabelTable | None`
- Purpose and contract: load subject-level multilabel disease-detection sidecars, requiring `disease_columns.txt` plus exact `[key_column] + disease_columns` columns in `label_index.csv` and `has_label_index.csv`.
- Important inputs/outputs: typed multilabel config plus optional expected output dim in; immutable `MultilabelLabelTable` out, or `None` when no config is supplied.
- Side effects: reads disease-column and sidecar files from disk.
- Key callers/callees: callers are root and variant PSG/Kaldi datasets and preset generation; callees include `load_multilabel_disease_columns`, `_load_sidecar`, and `normalize_multilabel_key`.
- Reuse guidance: use this function for every multilabel sidecar read.
- Duplication risk notes: disease-list validation, sidecar header validation, mask validation, duplicate-key checks, and output-dim parity belong here.

## `data.multilabel.attach_multilabel_metadata` and `stack_multilabel_metadata`

- File: `data/multilabel.py`
- Signatures:
  - `attach_multilabel_metadata(metadata: dict[str, Any], key_value: Any, labels: MultilabelLabelTable) -> None`
  - `stack_multilabel_metadata(samples: list[Any], expected_output_dim: int | None = None, key_column: str | None = None) -> dict[str, Any]`
- Purpose and contract: attach per-key `disease_label` and `has_label` vectors to sample metadata, then stack those vectors into batch tensors while preserving the configured subject key.
- Important inputs/outputs: row key or sample list in; metadata mutation or tensor/list batch fields out.
- Side effects: `attach_multilabel_metadata` mutates the sample metadata dict.
- Key callers/callees: callers are root and variant PSG/Kaldi datasets plus `DefaultDataset.dataloader`; callee is `normalize_multilabel_key`.
- Reuse guidance: use these helpers instead of hand-stacking multilabel disease metadata in dataset or trainer code.
- Duplication risk notes: presets built without multilabel metadata must fail fast here rather than silently falling back to ordinary metadata labels.
...
- Purpose and contract: build the same runtime batch contract as `PSGPretrainDataset` from a Kaldi `manifest.json` format v2 root rather than NPZ/preset inputs, including survival or multilabel sidecar attachment when configured.
- Important inputs/outputs: configured channels, optional survival or multilabel label config/output dim, Kaldi root, split manifest, and split name in; dataset instance out.
- Side effects: reads `manifest.json`, selects the requested split, builds channel specs, reads the split CSV, creates `SampleIndex` records with `payload["available_channels"]` and optional survival/multilabel metadata, and opens Kaldi readers lazily.
...
- Purpose and contract: canonical runtime collate path. It delegates batch channel/source selection to `_select_batch_channels`, reads NPZ slices or storage-hook tokens, tokenizes, builds masks, pads sequences, constructs `token_start`, stacks ordinary plus survival/multilabel metadata tensors, builds `w/h`, and selects the correct sampler, including optional binary-label weighted random sampling for train-only downstream imbalance runs.
