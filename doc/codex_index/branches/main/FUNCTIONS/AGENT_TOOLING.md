...
## `agent_tools.hparam_runtime.reconcile_hparam_launch_artifacts`

- File: `agent_tools/hparam_runtime.py`
- Signature: `reconcile_hparam_launch_artifacts(plan_dir: str | Path, started_keys: set[tuple[str, str]]) -> list[dict[str, Any]]`
- Purpose and contract: rebuild launch/status mirrors and missing `run_launched` events from plan-ordered canonical rows after an interrupted launch report. Event recovery is idempotent, including write-then-raise retries, and this path never starts a process or changes canonical status.
- Reuse guidance: use only after canonical state proves which runs started; do not call execute launch again to repair reporting artifacts.
...
- Purpose and contract: canonicalize the local plan path, preflight the current managed hparam plan, validate the complete candidate table against the workspace before filtering ownership-valid other-step or earlier same-step plan rows, require all registered owner plans in the step to share the caller's frozen selection metric and mode, require every retained row to have a positive integer rank and every checkpoint path to be a direct child of its frozen checkpoint directory, then apply top-k in numeric rank order or retain the original row order when all candidates were explicitly requested before preflighting all configs/manifest/script outputs; frozen run fields come from each candidate's owning plan.
...
- Purpose and contract: canonicalize the local plan path, preflight the current managed hparam plan, validate the complete candidate table plus checkpoint ownership before filtering ownership-valid other-step or earlier same-step plan rows, require all registered owner plans in the step to share the caller's frozen selection metric and mode, require every retained row to have a positive integer rank, then apply top-k in numeric rank order or retain the original row order when all candidates were explicitly requested before preflighting all configs/manifest/logits/script outputs; frozen run fields come from each candidate's owning plan, and dry-run replay scripts persist absolute plan/candidate-table arguments with the repository cwd/PYTHONPATH bootstrap.
...
- Purpose and contract: canonicalize the local plan path, preflight the current managed hparam plan, validate candidate ownership, positive integer ranks, and same-step owner-plan selection contracts, then require both validation and test predictions/logits, fit a binary F1 threshold on validation data, and apply it to test data.
...
- Purpose and contract: canonicalize the local plan path, preflight the current managed hparam plan, validate candidate ownership, positive integer ranks, and same-step owner-plan selection contracts, then average binary prediction probabilities across the retained registered-plan candidates, optionally searching combinations.
...
- Purpose and contract: perform one adaptive iteration: preflight deterministic outputs, monitor/digest, and suggest in preview mode; with `execute=True`, require the complete prospective replacement round to fit `max_rounds` and `max_runs_total`, plan and register it, then first launch against real free capacity. Prospective rounds use the next number above both registered rows and existing `round_NNN` directories, so an uncommitted failed directory remains an untouched single-use artifact and the next step creates a fresh round; `max_rounds` counts committed rounds rather than numeric gaps. Planning, registration, stop, launch, and zero-start failures preserve the prospective canonical rows in their actual `planned`, `pending`, `launch_failed`, or observed terminal state; they never manufacture `superseded` for an uncommitted round. If a launcher raises after PID creation but before status commit, adaptive reconciles that PID through the precommitted execution identity, then requires no-start mirror and idempotent `run_launched` recovery before writing `launch_round`. Failed canonical/artifact recovery leaves the prospective rows unchanged, and later execute steps reject any uncommitted round with PID, uncertain PID, active, or terminal execution evidence. If replacements remain pending in a full pool, stop at most one evidence-qualified bad running row before an ordinary launch retry and require a newly confirmed replacement before any further retirement; apart from the single row drained for the current launch attempt, confirmed replacements and retired bad rows remain one-to-one. The first recovered or directly confirmed replacement commits the round even when another replacement records `launch_failed`, after which only the previous current round's pending runs may be superseded. Launcher failures report confirmed stopped runs and superseded current pending runs separately; an existing commit is never rolled back.
