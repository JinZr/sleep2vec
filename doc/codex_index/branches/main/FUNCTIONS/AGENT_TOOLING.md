...
- Purpose and contract: run consultation, workspace containment, hparam/final-test, command-support, and output guards without writing files.
...
- Purpose and contract: validate the current-format registered plan, non-completed experiment, all managed tables, and every frozen config/script hash before creating a run directory, starting any process, or writing launch state; canonical workspace and local status are merged through the shared reducer before launch eligibility is decided.
...
- Purpose and contract: reduce `canonical run_manifest.tsv -> local run_status.tsv -> current observation` into one final row per run; launch rows contribute execution fields but never status, so missing or stale launch evidence cannot regress canonical state.
...
- Purpose and contract: merge canonical and local prior state, fail before PID access or writes when the canonical result is terminal, otherwise terminate one manifest-recorded PID and reduce one `stopped` observation with its non-empty reason.
...
- Purpose and contract: rank runs by the validation metric and direction frozen in the recipe, record fixed checkpoint paths, and validate every existing experiment-wide ranking row against canonical ownership before preserving rows from other steps.
...
- Purpose and contract: canonicalize the local plan path, preflight the current managed hparam plan, validate the complete candidate table against the workspace before other-step filtering, require rank/top-k filtering to retain a candidate, then create locked final/external inference configs and commands for current-plan candidates; frozen run fields come from the plan.
...
- Purpose and contract: canonicalize the local plan path, preflight the current managed hparam plan, validate the complete candidate table against the workspace before other-step filtering, require rank/top-k filtering to retain a candidate, then prepare or execute inference commands that export validation and optional test logits for current-plan candidates; frozen run fields come from the plan, and dry-run replay scripts persist absolute plan/candidate-table arguments with the repository cwd/PYTHONPATH bootstrap.
...
- Purpose and contract: keep `merge_run_row` as the pure status reducer with terminal precedence and the minimal active-to-scheduled regression guard, use `validate_frozen_run_update` at pre-side-effect boundaries, and let `merge_run_manifest` own local and SSH canonical persistence after re-reading current state. The validator's explicit checkpoint-ownership mode additionally requires each non-empty evidence `checkpoint_path` to be a direct child of the run's frozen `checkpoint_dir`; empty checkpoint evidence remains valid. The owner verifies every newly introduced key against the experiment id in the workspace's authoritative manifest, rejects changes to registered identity, semantic parameters, snapshots, hashes, and artifact paths, returns the complete rows actually committed, and passes those rows directly to `write_run_matrix` without a second manifest read.
...
## `agent_tools.manifests.read_rows` and `agent_tools.experiment_io.path_exists_at`, `read_rows_at`, and `write_rows_at`

- Files: `agent_tools/manifests.py`, `agent_tools/experiment_io.py`
- Signatures: `read_rows(path: str | Path, *, require_managed_identity: bool = False) -> list[dict[str, str]]`; `path_exists_at(path: str | Path, *, remote: str | None = None) -> bool`; `read_rows_at(path: str | Path, *, remote: str | None = None, require_managed_identity: bool = False) -> list[dict[str, str]]`; `write_rows_at(path: str | Path, rows: list[dict[str, Any]], *, remote: str | None = None) -> None`
- Purpose and contract: provide consistent local/SSH delimiter, quoting, and timeout behavior. `path_exists_at` distinguishes confirmed absence. Optional generic reads may represent genuinely missing input as `[]`, but a dangling local symlink is an existing corrupt artifact and fails on read; managed-table mode additionally rejects existing empty/headerless, duplicate-header, removed-format, missing-identity-header, malformed, and non-rectangular tables while accepting a current `step_id`/`run_id` header with zero rows. Transport, timeout, target-type, and read failures fail closed.
...
- Purpose and contract: validate the explicit YAML spec, all five existing ownership fields including root, and current-format managed tables before creating any local or remote experiment directories; only a genuinely empty target may be initialized, and initialization creates the valid header-only canonical run manifest before any run registration.
