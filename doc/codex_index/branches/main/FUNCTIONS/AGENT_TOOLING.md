...
- Signature: `launch_hparam_runs(plan_dir: str | Path, *, dry_run: bool = True, retiring_run_keys: set[tuple[str, str]] | None = None) -> Path`
- Purpose and contract: validate the current-format registered plan, non-completed experiment, all managed tables, every frozen config/script hash, canonical/mirror/report/event and per-run log/PID targets, plus the single-use frozen runtime/checkpoint roots before creating a run directory, starting any process, or writing launch state. `execution.gpus_per_run` requires a non-empty physical pool from `execution.gpu_pool` or `runtime.devices`; both consultation and runtime reject an unmanaged implicit pool. Canonical status decides launch eligibility, local status is mirror-only, and launch rows contribute only the shared execution-evidence allowlist. Adaptive replacement may pass evidence-qualified retiring run keys to borrow one temporary launch slot only from an exact matching GPU group whose post-retirement load remains valid; ordinary launch callers do not supply this internal handoff input. SSH execute validates the same paths on the execution host; dry-run performs no remote output probe.
- Important inputs/outputs: plan directory, dry-run flag, and optional adaptive-only retiring run keys in; `launch_manifest.tsv` path out.
...
- Purpose and contract: perform one adaptive iteration: preflight deterministic outputs, monitor/digest, and suggest in preview mode; with `execute=True`, require the complete prospective replacement round to fit `max_rounds` and `max_runs_total`, plan and launch it, then commit the round with the existing `launch_round` event only after the launcher returns and at least one replacement run has started. Evidence-qualified bad running rows may lend one temporary slot on their exact GPU group so a full default pool can complete that handoff; those rows are stopped only after the commit. Current pending runs are likewise superseded only after that commit. Failed planning, registration, launch, or zero-start confirmation leaves the prior round current; partial output is not repaired or treated as committed.
