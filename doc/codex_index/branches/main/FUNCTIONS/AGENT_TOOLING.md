...
- Purpose and contract: perform one adaptive iteration: preflight deterministic outputs, monitor/digest, and suggest in preview mode; with `execute=True`, require the complete prospective replacement round to fit `max_rounds` and `max_runs_total`, plan and register it, then first launch against real free capacity. If replacements remain pending in a full pool, stop at most one evidence-qualified bad running row before an ordinary launch retry and require a newly confirmed replacement before any further retirement; apart from the single row drained for the current launch attempt, confirmed replacements and retired bad rows remain one-to-one. The first replacement confirmed `launched` or `running` commits the round through the existing `launch_round` event, after which current pending runs may be superseded. If the launcher raises after its canonical write, a new start still commits only when that write added no `launch_failed`; a new failure prevents a new commit, while an existing commit is never rolled back. A pre-drain failure with no canonically confirmed replacement leaves old runs unchanged and the prior round current. A zero-start failure after a drain keeps the stopped run and prospective run states as recorded while the prior round remains current; a later failure after commit keeps the new round current and leaves remaining old runs unchanged. Failed prospective directories are single-use and are neither rolled back nor retried automatically.
