...
- `python -m agent_tools experiment-init --run-dir <dir> --name <id> [--remote <host>]`
...
Optional hparam `execution` fields enable active orchestration after a plan has been generated. `hparam-launch` reads the generated `plan.json` and trial scripts, assigns GPUs from `execution.gpu_pool`, wraps commands with optional `conda run`, W&B project/group environment variables, nohup logs, and PID files, then writes `launch_manifest.tsv` and `trial_status.tsv`. Dry-run is the default; `--execute` is required to start processes. `execution.path_context=remote` with `path_validation=defer|ssh` lets doctor/plan avoid local false negatives for remote absolute paths; hparam config generation still requires local YAML content when config copies must be written. For `execution.target=ssh`, relative `log_dir` and `pid_dir` are remote paths under remote `workdir`, remote log/PID directories are created through SSH before `nohup`, and launch manifests store remote log/PID paths instead of creating those directories locally. `hparam-stop` only terminates a PID recorded in the launch manifest. `hparam-monitor` updates trial state from recorded PIDs, logs, run manifests, W&B summaries, and checkpoint directories; `--health` adds opt-in short-command GPU, `/proc/<pid>/io`, log-age, checkpoint-count, and progress-file checks. Remote probes use short SSH calls with timeout, and `status/progress.json` contributes healthy status only when it is fresh or its processed count advanced since the previous monitor run. `hparam-select` ranks trials by validation metrics and writes fixed `epoch=XX.ckpt` checkpoint paths rather than moving best aliases. `hparam-checkpoint-scan` scans local W&B history and checkpoint directories to rank fixed epoch checkpoints. `hparam-external-eval` remains locked unless `--unlock-final-test` is present, copies selected trial YAMLs while replacing data entry fields, and preserves base infer runtime settings plus selected-row `runtime.*` overrides; `--top-k` and `--all-candidates` allow evaluating more than rank 1. `hparam-export-logits` remains locked for test export unless `--unlock-final-test` or `--skip-test` is present, writes split-specific copied configs plus `logits_export_manifest.tsv`, and can execute the rendered inference commands. `hparam-threshold` fits a validation F1 threshold and applies it to test logits. `hparam-ensemble --search-combinations` enumerates probability-average model combinations and ranks them by the requested metric.

Experiment monitoring commands provide a unified layer above hparam-specific tooling without changing training entrypoints. `experiment-init` creates `experiment_manifest.tsv` plus `reports/` and `wandb/history/`. `experiment-wandb-sync` uses `wandb.Api()` summary and history only, then writes `wandb/runs.tsv`, `wandb/summaries.jsonl`, per-run history CSVs, `metrics_manifest.tsv`, and W&B fields in `run_manifest.tsv`; W&B browser pages are not treated as numeric data sources. `experiment-index-checkpoints` writes `checkpoint_manifest.tsv` from local checkpoint trees or, with `--remote`, from a short SSH `find` under the remote run directory. `experiment-monitor` merges `run_manifest.tsv`, `launch_manifest.tsv`, `trial_status.tsv`, progress, log tail, PID, GPU, IO, checkpoint, and synced metric state into `run_manifest.tsv` and `reports/monitor.md`. `experiment-rank` ranks `metrics_manifest.tsv` rows into `candidate_ranking.tsv` and `reports/wandb_rank.md`. When `--remote <host>` is present, experiment init, W&B sync outputs, checkpoint indexing, monitoring, and ranking read/write the given `<run-dir>` on that host through short SSH calls; when Codex is running directly on the server, omit `--remote` and use the same commands against server-local `/wujidata/...` paths.
...
- Change hparam launch/monitor/select/export: `agent_tools/hparam.py`.
- Change experiment-level W&B/checkpoint/run monitoring: `agent_tools/experiments.py`.
