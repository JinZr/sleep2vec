...
| Task semantics | `sleep2vec.common.apply_task_flags` | Single source for built-in labels and custom-task validation | Duplicated task/type/monitor logic |
...
| Finetune data loaders | `sleep2vec.utils.get_finetune_dataloaders` | Owns split/source choices and stage5 pseudo-channel behavior | Hand-rolled finetune loader creation |
...
| Downstream metric reduction | `sleep2vec.metrics.compute_downstream_metrics` | Single metric reducer for classification/regression outputs | Per-stage custom metric calculations |
