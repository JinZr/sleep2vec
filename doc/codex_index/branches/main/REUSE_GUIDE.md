...
| sleep2stat execution loop | `sleep2stat.core.pipeline.run_pipeline` | Owns dry-run, single-use output checks, chunking, analyzer preparation, reducer fallback, failure accounting, progress, and manifest writes | A parallel execution manager in `agent_tools` or scripts |
...
| sleep2stat bundle writing | `sleep2stat.io.writers.AnalysisBundleWriter` | Owns single-use per-record sidecars, global shards, cumulative summaries, failures, progress, and manifest schema | Writing bundle files directly from analyzers or CLI code |
...
- Reuse `AnalysisBundleWriter` for output files, single-use output checks, current-run failures, and global table rebuilds.
