...
| Finetune YAML parsing | `sleep2vec.config.load_finetune_config` | Centralizes task, head, survival/multilabel sidecars, layer-mix, LoRA/DoRA hyperparameters, evaluation-visualization, and model-averaging parsing | Entry-point specific YAML parsing |
...
| Multilabel disease sidecar loading | `data.multilabel.load_multilabel_label_table`, `attach_multilabel_metadata`, and `stack_multilabel_metadata` | Single contract for disease columns, label/has-label sidecars, key normalization, and batch metadata stacking | Wide ordinary metadata labels or inline CSV joins in trainers, metrics, or preprocessing scripts |
...
| Multilabel disease reduction | `Sleep2vecFinetuning._aggregate_multilabel_records` and `compute_multilabel_classification_metrics` | Aggregates repeated windows to one subject-level mean-logit row, masks invalid cells with `has_label`, and computes micro/macro AUROC/AUPRC plus thresholded micro metrics | Disease-detection metrics inside loaders or result writers |
...
| Inference W&B artifacts | `sleep2vec.infer._log_inference_outputs_to_wandb` | Logs metrics plus prediction and survival/multilabel per-disease row counts, and uploads the metrics, predictions, optional per-disease metrics, manifest, and overview files together | W&B upload logic in result writers or variant-specific one-offs |
...
| Multilabel per-disease CSV output | `sleep2vec.results.save_multilabel_per_disease_metrics_csv` | Preserves rank-zero gating, lockfile semantics, schema expansion, and inference/finetune metadata for disease-level detection metrics | Ad hoc disease-metric CSV writers |
...
| Agent workflow support | Public facades in `agent_tools.plans`, `agent_tools.decisions`, `agent_tools.hparam`, and `agent_tools.experiments`; responsibility owners in `decision_*`, `plan_*`, `hparam_*`, `run_*`, and `experiment_*`; plus `agent_tools.recipes`, `agent_tools.experiment_workspace`, `agent_tools.adaptive_hparam`, and `agent_tools.progress` | Centralizes managed experiment ownership, semantic run identity, frozen plan snapshots, recipe loading, stop-and-consult gates, orchestration, monitoring, and machine-readable progress while keeping leaf dependencies one-way | A second experiment directory convention, training entrypoint, natural-language-only policy, or new cross-module private helper |
...
### If you are changing multilabel disease-detection behavior

- Reuse `data.multilabel` for sidecar loading and metadata stacking.
- Keep task schema in `sleep2vec.config.MultilabelConfig` and `finetune.multilabel`.
- Keep masked BCE, subject-level duplicate aggregation, micro/macro metrics, per-disease metric rows, and disease-vector prediction rows in `Sleep2vecFinetuning` / `sleep2vec.metrics`.
- Mirror root changes into `sleep2vec2` and `sleep2expert` when the contract is shared.
...
- Reuse `agent_tools.plans.build_context`, `agent_tools.plans.build_plan`, `agent_tools.recipes`, and `agent_tools.decisions.evaluate_consultation_gates` as the stable public entrypoints for context, plan, recipe, and stop-and-consult behavior. Use the pure `plans.preflight_plan` before an adjacent workflow mutates workspace or run state. Add supported rendered `runtime` fields to the existing decision owner rather than letting unknown keys fall through to renderer defaults.
- Put decision types in `decision_models`, local/SSH path checks in `decision_paths`, task rules in `decision_rules`, and tuning rules in `decision_hparam`; keep the hparam `adaptive`, `replacement`, and `suggest` field vocabularies closed there, and do not import private helpers from the `decisions` facade.
- Put command rendering in `plan_rendering`, context/index/Markdown logic in `plan_context`, and grid/final-test/frozen-plan logic in `plan_hparam`; keep `_commands_for_recipe` in `plans` as the only task dispatcher. Reuse `plan_rendering.script_lines` for generated cwd/PYTHONPATH and canonical non-hparam lifecycle shell instead of repeating it in a dispatcher.
- Reuse `agent_tools.experiment_workspace.canonical_local_experiment_root` at every public local management boundary, and persist repository-owned plan/run/postprocess/adaptive/event locators as absolute paths; do not apply that normalization to user-authored semantic data or checkpoint inputs. Use `read_managed_yaml_mapping` for authoritative experiment/step YAML and `read_step_manifest` plus `merge_step_manifest` for every `step.yaml` producer. Use `initialize_run_manifest` only during fresh workspace initialization and `read_run_manifest` for every later canonical read; missing, valid header-only, and corrupt tables have distinct fail-closed semantics locally and over SSH. Use `read_rows(..., require_managed_identity=True)` for managed auxiliary evidence so removed or incomplete header-only tables cannot appear empty, and keep `step_id/run_id` headers when a managed writer has zero rows. Treat `run_manifest.tsv` as authoritative durable state: mutation-facing tracking produces source-allowlisted observations, `merge_run_manifest` is the only commit path, and mirrors, matrices, reports, and events consume its returned rows. Prove external ownership before allowlisting or filtering: W&B requires a matching experiment id or unique-version fallback; launch/status/metric/checkpoint/ranking/registry/candidate rows require canonical workspace membership and frozen-field agreement; retained candidates additionally require current-plan membership. Reuse `managed_run_parameters`, explicit table cardinality, `merge_run_row`, `run_artifacts.read_hparam_plan`, `run_artifacts.find_run_manifest`, and `run_evidence` rather than creating parallel recipe/runtime readers, reducers, or path inference.
- Reuse the public `agent_tools.hparam` and `agent_tools.experiments` entrypoints for commands; change their implementations in `hparam_runtime`, `hparam_selection`, `hparam_postprocess`, `experiment_io`, or `experiment_tracking` according to responsibility. Reuse `experiment_io.read_rows_at(..., strict=True)` for non-run authoritative tables and `validate_managed_output_paths` for local/SSH managed-output preflight instead of adding command-local parsers or alias checks. Route PID evidence through `run_evidence.read_pid`; stop supplies the frozen script so process identity is proven before signaling. Reuse `agent_tools.adaptive_hparam` and `agent_tools.progress` for adaptive tuning and progress reporting.
...
12. Inference metrics, overview rows, predictions, survival/multilabel per-disease metrics, manifests, and W&B artifacts share one `prediction_run_id`; update `sleep2vec.results`, `sleep2vec.sleep2vec_inference`, and inference W&B logging together instead of adding a parallel export path.
...
17. Multilabel disease labels are subject-level sidecar metadata; aggregate duplicate windows by multilabel key before validation/test metrics and prediction export, and keep disease vector order from `disease_columns.txt`.
18. LSTM temporal aggregation requires at least one valid token per sample; do not mask it away silently or pool zero-length rows.
19. sleep2expert compact export rewrites both config and checkpoint layout; do not mix compact checkpoints with the original expert id mapping.
