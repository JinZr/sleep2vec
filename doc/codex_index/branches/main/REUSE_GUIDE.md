...
- Reuse `agent_tools.plans.build_context`, `agent_tools.plans.build_plan`, `agent_tools.recipes`, and `agent_tools.decisions.evaluate_consultation_gates` as the stable public entrypoints for context, plan, recipe, and stop-and-consult behavior. Use the pure `plans.preflight_plan` before an adjacent workflow mutates workspace or run state.
- Put decision types in `decision_models`, local/SSH path checks in `decision_paths`, task rules in `decision_rules`, and tuning rules in `decision_hparam`; do not import private helpers from the `decisions` facade.
