...
3. `sleep2stat.core.pipeline.run_pipeline` prepares an `AnalysisBundleWriter`, rejects non-empty output directories, prepares enabled analyzers, executes analyzers chunk by chunk, applies reducers, and writes progress, failures, manifests, and result bundles.
...
5. `sleep2stat.io.writers.AnalysisBundleWriter` owns per-record `events.csv.gz`, `night_stats.json`, `arrays.npz`, `result_manifest.csv`, global table shards, rebuilt cohort tables, current-run failures, and run manifests.
