...
| Standalone high-resolution variant | `sleep2vec_hires/`, `configs/sleep2vec_hires/` | Package-local mirror seeded from `sleep2vec2` as the boundary for future high-resolution token data/model contracts | `sleep2vec2` dense mirror at creation time, local package imports | High-resolution token semantics, recipes, data/preprocess changes | Starts as a direct mirror; keep future high-resolution divergence package-local |
...
- `sleep2vec2`, `sleep2vec_hires`, and `sleep2expert` edits should preserve package-local imports and mirror tests; do not route them through root `data` or `preprocess`.
