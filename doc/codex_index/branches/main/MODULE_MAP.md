...
| Agent tooling | `agent_tools/`, `skills/`, `recipes/`, `agent_policies/`, `doc/agent_contracts/` | Summarize repo/config/index/preset state, validate skills and recipes, enforce stop-and-consult gates, write context bundles, generate safe command plans, orchestrate hparam trials, monitor experiments, and write progress artifacts | yaml, pandas, standard library, optional wandb | New agent task surfaces and policy gates | Must call canonical runtime/preprocess entrypoints rather than replacing them |
