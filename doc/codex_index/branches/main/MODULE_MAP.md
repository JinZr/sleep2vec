...
| --- | --- | --- | --- | --- | --- |
| Config schema and built-in task semantics | `sleep2vec/config.py`, `sleep2vec/common.py` | Parse YAML, validate required blocks, define built-in labels plus survival and multilabel disease tasks, bind config state into CLI namespaces, persist run artifacts, normalize NPZ/Kaldi backend settings, and carry LoRA/DoRA adapter settings | `yaml`, dataclasses | New config fields, new built-in tasks, survival/multilabel sidecars, new eval-visualization knobs, new adapt fields, new data backends | Canonical parse and task-semantics boundary |
...
| Downstream model and head composition | `sleep2vec/downstream_model.py`, `sleep2vec/downstreams/` | Temporal aggregation including LSTM pooling, channel fusion, downstream heads, layer mix, pretrained loading, LoRA insertion | `peft`, downstream registries | New heads, fusion modes, adapter behavior | Canonical downstream feature path |
| Lightning runtime | `sleep2vec/sleep2vec_modelling.py`, `sleep2vec/sleep2vec_finetuning.py`, `sleep2vec/sleep2vec_adaptation.py`, `sleep2vec/schedulers.py` | Training-loop glue, optimizer schedule, diagnostics, model averaging, AHI metrics, Cox survival loss/c-index, multilabel disease metrics, eval visualization logging, staged adaptation optimizer groups | PyTorch Lightning, loss/metrics, callbacks | New trainer behavior | Keep optimizer groups in runtime modules and shared LR schedule in package-local scheduler helpers |
...
| Dataset core | `data/default_dataset.py`, `data/psg_pretrain_dataset.py`, `data/survival.py`, `data/multilabel.py` | Build `SampleIndex` windows, validate/load presets, attach survival or multilabel sidecar metadata, materialize batches, backfill built-in AHI metadata, and choose samplers | `data.utils`, `data.metadata`, `data.samplers` | New dataset formats or collate behavior | Main data contract boundary |
...
| Agent tooling | `agent_tools/`, `skills/`, `recipes/`, `agent_policies/`, `doc/agent_contracts/` | Own managed experiment workspaces, summarize repo/config/index/preset/sleep2stat state, validate skills and recipes, enforce stop-and-consult gates, generate frozen command plans, orchestrate named hparam runs, monitor experiments, and write progress/report artifacts | yaml, pandas, standard library, optional wandb | New agent task surfaces, experiment lifecycle contracts, and policy gates | Must call canonical runtime/preprocess/sleep2stat entrypoints rather than replacing them |
| sleep2stat analysis bundles | `sleep2stat/config.py`, `sleep2stat/cli.py`, `sleep2stat/core/`, `sleep2stat/analyzers/`, `sleep2stat/reducers/`, `sleep2stat/io/`, `sleep2stat/plot.py`, `configs/sleep2stat/`, `tests/sleep2stat/test_sleep2stat_*.py` | Validate analysis YAMLs, load NPZ/Kaldi records, run model/YASA/SpO2 analyzers, reduce derived metrics, write single-use per-record and global bundles, and render record/cohort plots | root `data` package, variant finetune modules through analyzer namespace, optional YASA/MNE, pandas/NumPy/matplotlib | New analyzer types, reducers, bundle outputs, plot panels, and sleep2stat recipe behavior | Derived analysis runtime; do not add trainer behavior here |
...
| Config recipes | `configs/` | Encode model/head/task variants, preset-build policy, adapt recipes, example recipes, hypnodata examples, standalone `sleep2vec2` recipes, and current `sleep2expert/moe` recipes | Config parser + entrypoints | New recipe variants | Folder names are not always semantically authoritative |
| Tests | `tests/` | Pin config, registry, LoRA adapter, survival/Cox, AHI/specificity metric, checkpoint, result CSV, inference W&B artifact, preset-build, pair-sampler, hypnodata, variant, sleep2stat, and visualization contracts | pytest | New contract coverage | Many important contracts live here now; top-level tests are grouped by area subdirectory |
| Formatting wrapper | `utils/style_check.sh` | Run `isort`, `black`, `flake8` over repo | Python env toolchain | None | Lint wrapper only |
| Standalone dense variant | `sleep2vec2/`, `configs/sleep2vec2/` | Package-local mirror of root runtime/data/preprocess plus standalone RoFormer and survival/LSTM mirrors, with tests for namespace isolation, Kaldi backend, LoRA/DoRA adapter parity, and dense parity | root contracts for behavior parity, local package imports | Variant-specific dense recipe changes | LoRA/DoRA support follows the root downstream adapter contract |
| Standalone MoE variant | `sleep2expert/`, `configs/sleep2expert/` | Package-local mirror plus MoE RoFormer, survival/LSTM mirrors, routing regularization, MoE finetune tuning, LoRA/DoRA adapter support, checkpoint expansion, runtime route filtering, compact subnetwork export, routing export, and routing visualizations | `sleep2expert.config`, `sleep2expert.backbones.roformer.moe`, `sleep2expert.losses.moe_regularization`, `sleep2expert.export_subnetwork` | MoE architecture, tuning, routing analysis, compact export | Router LoRA is intentionally unsupported; adapter params use a separate MoE finetune LR group |
...
- `infer.py` -> `sleep2vec.results.prepare_inference_result_paths` + `save_result_csv` + `save_prediction_csv` + `save_survival_per_disease_metrics_csv` + `save_multilabel_per_disease_metrics_csv` + `save_inference_manifest`
...
- `Sleep2vecFinetuning` survival path -> `data.survival` metadata vectors and optional disease names + `sleep2vec.losses.cox.CoxPHLossVectorized` + `metrics.compute_survival_c_index_by_disease`
- `Sleep2vecFinetuning` multilabel disease path -> `data.multilabel` metadata vectors and disease names + masked BCE + `metrics.compute_multilabel_classification_metrics`
...
- `agent_tools.cli` -> public facades in `agent_tools.decisions`, `agent_tools.plans`, `agent_tools.hparam`, and `agent_tools.experiments`, plus summary/adaptive/progress modules; generated scripts call canonical variant package and `preprocess` entrypoints
- `agent_tools.decision_models` -> `agent_tools.decision_paths`, `agent_tools.decision_rules`, and `agent_tools.decision_hparam` -> `agent_tools.decisions`; the facade owns source resolution, aggregate consultation status, and base-finetune recursion
- `agent_tools.plan_rendering` -> `agent_tools.plan_context` and `agent_tools.plan_hparam` -> `agent_tools.plans`; `_commands_for_recipe` remains the single task dispatcher in the public orchestration module
- `agent_tools.manifests` -> `agent_tools.run_artifacts` -> `agent_tools.run_evidence`, `agent_tools.hparam_runtime`, `agent_tools.hparam_selection`, and `agent_tools.hparam_postprocess` -> the public `agent_tools.hparam` facade; every hparam writer verifies its actual frozen config/script files through `run_artifacts.read_hparam_plan`, managed auxiliary readers retain identity-header semantics, and postprocessing canonicalizes its local plan path before persistence
- `agent_tools.experiment_io` owns local/SSH strict table reads and managed-output path preflight; together with `agent_tools.run_artifacts` and `agent_tools.run_evidence` it feeds `agent_tools.experiment_tracking` and the seven public lifecycle/tracking commands in `agent_tools.experiments`
- `agent_tools.experiment_workspace` owns canonical local root representation, absolute repository-owned management locators, duplicate-key-aware authoritative YAML, strict step/run manifest readers and fresh run-manifest initialization, the canonical step envelope, exact `(step_id, run_id)` resolution, external ownership proof, raw managed-parameter projection, explicit table-cardinality validation, frozen managed-row fields, the shared status reducer, and local/SSH canonical commits; mutation-facing tracking emits observations only, and every mirror, matrix, report, or event consumes the rows returned by that commit
...
- `sleep2expert.export_subnetwork` -> `sleep2expert.config.load_*_config` + `resolve_route_expert_ids` + checkpoint state-dict rewrite
...
- LoRA/DoRA adapter edits usually span `sleep2vec/config.py`, `sleep2vec/common.py`, `sleep2vec/downstream_model.py`, `sleep2vec/sleep2vec_finetuning.py`, and `tests/models/test_downstream_separate_adapters.py`; package-local variant edits mirror those files under `sleep2vec2/` or `sleep2expert/`.
- Survival or multilabel disease task edits usually span `sleep2vec/config.py`, `sleep2vec/common.py`, `data/survival.py` or `data/multilabel.py`, `data/psg_pretrain_dataset.py`, `data/kaldi_psg_dataset.py`, `sleep2vec/sleep2vec_finetuning.py`, `sleep2vec/metrics.py`, preprocessing converters, and variant mirrors.
...
- sleep2stat config/parser edits usually span `sleep2stat/config.py`, `configs/sleep2stat/`, `agent_tools/configs.py`, `utils/check_configs.py`, and `tests/sleep2stat/test_sleep2stat_config.py`.
...
- sleep2stat metric edits should stay in the owning analyzer or reducer and update the focused test file: `tests/sleep2stat/test_sleep2stat_analyzers.py`, `tests/sleep2stat/test_sleep2stat_reducers.py`, or `tests/sleep2stat/test_sleep2stat_writers.py`.
- Agent-facing workflow edits should stay in `agent_tools/`, `skills/`, `recipes/`, `agent_policies/`, or `doc/agent_contracts/`; edit the responsibility owner above rather than growing the public facade, and do not add new model-training runtimes.
...
- `sleep2expert` MoE edits usually span `sleep2expert/config.py`, `sleep2expert/backbones/roformer/moe.py`, `sleep2expert/pretrain_model.py`, `sleep2expert/sleep2vec_modelling.py`, `sleep2expert/sleep2vec_finetuning.py`, and `sleep2expert/losses/moe_regularization.py`; MoE LoRA trainability and LR-scale grouping belongs in `sleep2expert/sleep2vec_finetuning.py`, while compact checkpoint/config export belongs in `sleep2expert/export_subnetwork.py`.
