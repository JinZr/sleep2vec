...
| --- | --- | --- | --- | --- | --- |
| Config schema and built-in task semantics | `sleep2vec/config.py`, `sleep2vec/common.py` | Parse YAML, validate required blocks, define built-in labels and survival tasks, bind config state into CLI namespaces, persist run artifacts, normalize NPZ/Kaldi backend settings, and carry LoRA/DoRA adapter settings | `yaml`, dataclasses | New config fields, new built-in tasks, survival sidecars, new eval-visualization knobs, new adapt fields, new data backends | Canonical parse and task-semantics boundary |
...
| Lightning runtime | `sleep2vec/sleep2vec_modelling.py`, `sleep2vec/sleep2vec_finetuning.py`, `sleep2vec/sleep2vec_adaptation.py`, `sleep2vec/schedulers.py` | Training-loop glue, optimizer schedule, diagnostics, model averaging, AHI metrics, Cox survival loss/c-index, eval visualization logging, staged adaptation optimizer groups | PyTorch Lightning, loss/metrics, callbacks | New trainer behavior | Keep optimizer groups in runtime modules and shared LR schedule in package-local scheduler helpers |
...
| Dataset core | `data/default_dataset.py`, `data/psg_pretrain_dataset.py`, `data/survival.py` | Build `SampleIndex` windows, validate/load presets, attach survival sidecar metadata, materialize batches, backfill built-in AHI metadata, and choose samplers | `data.utils`, `data.metadata`, `data.samplers` | New dataset formats or collate behavior | Main data contract boundary |
...
- Survival task edits usually span `sleep2vec/config.py`, `sleep2vec/common.py`, `data/survival.py`, `data/psg_pretrain_dataset.py`, `data/kaldi_psg_dataset.py`, `sleep2vec/sleep2vec_finetuning.py`, `sleep2vec/metrics.py`, preprocessing converters, and variant mirrors.
