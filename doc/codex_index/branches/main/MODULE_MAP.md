...
| Agent tooling | `agent_tools/`, `skills/`, `recipes/`, `agent_policies/`, `doc/agent_contracts/` | Summarize repo/config/index/preset state, validate skills and recipes, enforce stop-and-consult gates, write context bundles, and generate safe command plans | yaml, pandas, standard library | New agent task surfaces and policy gates | Must call canonical runtime/preprocess entrypoints rather than replacing them |
...
- `agent_tools.cli` -> `agent_tools.decisions` + summary modules + `agent_tools.plans`; generated scripts call canonical variant package and `preprocess` entrypoints
