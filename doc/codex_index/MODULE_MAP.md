...
| Metrics and diagnostics | [`sleep2vec/metrics/`](../../sleep2vec/metrics/), [`sleep2vec/diagnostics.py`](../../sleep2vec/diagnostics.py), [`sleep2vec/visualization/`](../../sleep2vec/visualization/) | Shared metric primitives, task-specific AHI and arousal reduction, diagnostics hooks, evaluation plots | reported metric or visualization contracts change |
...
| Agent control plane | [`agent_tools/`](../../agent_tools/), [`agent_tools/ARCHITECTURE.md`](../../agent_tools/ARCHITECTURE.md) | Consultation, context, plans, managed experiments, hparam lifecycle, progress, and summaries; research-log validation/publication lives in [`agent_tools/research_log.py`](../../agent_tools/research_log.py) | agent-facing contracts or experiment ownership changes |
| Runtime checkout refresh | [`agent_tools/runtime_sync.py`](../../agent_tools/runtime_sync.py), [`agent_tools/runtime_lock.py`](../../agent_tools/runtime_lock.py) | Inspect or fast-forward one clean checkout to `origin/main` and coordinate that update with process-start identity capture | rolling checkout or launch/update coordination changes |
| Managed scheduling | [`agent_tools/managed_scheduler.py`](../../agent_tools/managed_scheduler.py), [`agent_tools/slurm.py`](../../agent_tools/slurm.py) | Select direct or Slurm backend; own direct GPU/process lifecycle plus Slurm leaf-job artifacts, submission, observation, reconciliation, and cancellation | hparam or ordinary infer/evaluate Slurm lifecycle changes; external pipelines remain direct-only |
| Experiment evidence and projections | [`agent_tools/experiment_sources.py`](../../agent_tools/experiment_sources.py), [`agent_tools/experiment_tracking.py`](../../agent_tools/experiment_tracking.py) | Acquire W&B/checkpoint evidence, then validate and project canonical experiment status and rankings | evidence acquisition or experiment projection behavior changes |
| Managed evaluation pipeline | [`agent_tools/experiment_pipeline.py`](../../agent_tools/experiment_pipeline.py), [`agent_tools/experiment_pipeline_spec.py`](../../agent_tools/experiment_pipeline_spec.py), [`agent_tools/experiment_pipeline_cohort_selection.py`](../../agent_tools/experiment_pipeline_cohort_selection.py), [`agent_tools/experiment_pipeline_results.py`](../../agent_tools/experiment_pipeline_results.py), [`agent_tools/experiments.py`](../../agent_tools/experiments.py) | Validate declared specs before publication; wait for managed sources, freeze ranked checkpoints or candidates, run selection/report jobs, validate results, and finalize | `experiment-run`, external-matrix, cohort-selection, retry, result validation, or pipeline-resume behavior changes |
| Adaptive evidence and proposals | [`agent_tools/adaptive_evidence.py`](../../agent_tools/adaptive_evidence.py), [`agent_tools/adaptive_proposals.py`](../../agent_tools/adaptive_proposals.py), [`agent_tools/adaptive_hparam.py`](../../agent_tools/adaptive_hparam.py) | Read and rank canonical round evidence; validate immutable snapshots and bounded proposals, then orchestrate publication, preflight, registration, and launch | adaptive evidence, strategy, proposal protocol, or parameter-envelope semantics change |
...
- Built-in arousal uses one 1 Hz, four-column multi-hot target in fixed `[RES, SPONT, Limb, PLM]` order. NPZ and Kaldi both expose it as logical channel `arousal`; the data layer flattens each 30-second token second-major and subtype-minor to width 120 and carries total plus subtype ArI scalars and `tst` as metadata.
