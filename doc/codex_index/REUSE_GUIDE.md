...
| Rolling checkout refresh | `runtime-sync` in [`agent_tools/runtime_sync.py`](../../agent_tools/runtime_sync.py), coordinated by [`agent_tools/runtime_lock.py`](../../agent_tools/runtime_lock.py) | clone-per-commit heartbeat logic, reset-based updaters, or launcher-local Git mutations |
...
| Hparam selection, candidate resolution, and report | canonical selection fields and read-only top-K/all candidate resolution in [`agent_tools/hparam_selection.py`](../../agent_tools/hparam_selection.py), with checkpoint-test validation and best-candidate ordering in [`agent_tools/checkpoint_test_results.py`](../../agent_tools/checkpoint_test_results.py) and projection validation/rendering in [`agent_tools/experiment_tracking.py`](../../agent_tools/experiment_tracking.py) | postprocess-, pipeline-, event-, ranking-, status-, or finalizer-local selection logic |
...
| Managed run key extraction | `experiment_workspace.managed_run_key` for optional identity; `validated_run_key` for rows already validated by the canonical reader or validator | caller-local identity tuples or repeated full-row validation |
| Managed direct process identity and stopping | [`agent_tools/run_evidence.py`](../../agent_tools/run_evidence.py), consumed by the hparam and preset launch/stop entrypoints | PID-only checks or caller-local signals |
...
| Managed scheduler lifecycle | [`agent_tools/managed_scheduler.py`](../../agent_tools/managed_scheduler.py) with Slurm primitives in [`agent_tools/slurm.py`](../../agent_tools/slurm.py); shared scheduler queries stay within one round and frozen route, never launch/stop/reconciliation; snapshot hooks distinguish missing from empty snapshots and request writes only for present snapshots | hparam- or pipeline-local capacity, scheduler commands, observation, reconciliation, snapshot, start, or stop implementations |
...
- Keep step-based LR curves in each variant's `schedulers.py`; finetune's `configure_optimizers` owns scheduler selection, and its validation hook advances Plateau before checkpoint saving. Reuse the package-local scheduler argument validator before run-directory preflight and for model-only callers. WSD adds a stable phase to the existing warmup/decay path, while parameter-group LR ratios and checkpoint continuation remain runtime contracts.
...
- Keep proposal snapshots/envelopes and shared initial/proposal point validation in
  `adaptive_proposals`, round evidence reading and ranking in `adaptive_evidence`,
  and monitoring, publication, preflight, and lifecycle orchestration in `adaptive_hparam`.
  Reuse canonical round evidence for complete-history proposal
