...
- `hparam-stop` only stops a PID from the canonical run row, requires a reason, rejects launch evidence that drifts from the canonical execution identity before PID access, rejects an already-terminal canonical run, and records `stopped` only after the local signal or SSH command succeeds.
