...
Preset preparation routes `sleep2vec` to `preprocess/save_dataset_presets.py`, `sleep2vec2` to `sleep2vec2/preprocess/save_dataset_presets.py`, and `sleep2expert` to `sleep2expert/preprocess/save_dataset_presets.py`. Variant scripts reject root-only `manifest_output` and `write_sidecar_manifest` fields instead of falling back to the root runtime. `sex_age_baseline` is a standalone runtime variant but does not own preset generation. Missing, extra, or unsupported variant values block command generation rather than falling back to another namespace.

`pretrain` and `adapt` remain covered by their task skills and direct variant runtime CLIs; they are not runnable agent-tools recipe values because no recipe renderer exists for them.

Runnable non-hparam scripts for `finetune`, `infer`, `evaluate`, `preset_prepare`, and `sleep2stat` always use `REPO_ROOT` as cwd and PYTHONPATH, independent of the caller's current directory.
...
When a user-decision file selects a different finetune, inference, evaluation, or hparam config, that value is written to the canonical effective `inputs.config`, then passes config summary and consultation again before any workspace or run artifact is written. Hparam run snapshots, launch, selection, and postprocessing read that merged effective recipe; base/local copies remain source audit only. An explicit empty or `ASK_USER` decision remains unresolved; a missing, unreadable, non-finetune, or semantically blocked selected config cannot leave a partially initialized plan.
