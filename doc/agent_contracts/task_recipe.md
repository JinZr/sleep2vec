...
shorthand; the commit is a lowercase 40-character planned/baseline Git SHA.
...
planned/baseline `execution.runtime_commit`. Submission may use `target: local`
or `target: ssh`;
...
- `execution.python` names one target Python executable without whitespace,
  ...
  planned/baseline Git commit; launch records the actual commit separately.
