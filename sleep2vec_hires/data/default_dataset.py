...
from sleep2vec_hires.data.metadata import (
    build_w_h_age_sex_center,
    extract_binary_labels,
    make_weighted_sampler_from_labels,
    process_metadata,
)
from sleep2vec_hires.data.utils import filter_valid_sample_indices, load_builtin_ahi_metadata, load_npz
...
class DefaultDataset(BaseDataset):
    ...
    def __init__(
        ...
        if load_preset_path:
            ...
            # Presets built before the namespace fork pickle data.default_dataset.SampleIndex objects.
            self.data = [
                (
                    item
    ...
    def dataloader(self, device: str = "cpu") -> DataLoader:
        ...
        if allow_missing_channels and self.is_train_set:
            ...
            from sleep2vec_hires.data.samplers import PairFirstBatchSampler
        ...
        if allow_missing_channels and bucket_by_available_channels and sampler is None:
            from sleep2vec_hires.data.samplers import AvailableChannelsBucketBatchSampler
