...
def scaling_step(group, p, state, grad):
    ...
    try:
        ...
        scale_grads = torch.zeros(size_update_period, *param_rms.shape, dtype=torch.float, device=p.device)
...
class ScaledAdam(BatchedOptimizer):
    ...
    def _show_gradient_dominating_parameter(
        ...
        sorted_by_proportion = {
            k: v for k, v in sorted(all_sumsq_orig.items(), key=lambda item: item[1][0], reverse=True)
        }
...
class LRScheduler(object):
    ...
    def print_lr(self, is_verbose, group, lr):
        ...
        if is_verbose:
            logging.warning(
                f"Epoch={self.epoch}, batch={self.batch}: adjusting learning rate" f" of group {group} to {lr:.4e}."
            )
