...
def _require_channels(model_block: dict[str, t.Any]) -> t.List[ChannelConfig]:
    ...
    for item in channels_raw:
        ...
        if not isinstance(aliases, list) or not all(isinstance(alias, str) and alias for alias in aliases):
            raise ValueError(f"channel '{item.get('name', '?')}' aliases must be a list of non-empty strings.")
...
def _build_survival_config(raw: t.Any, task_cfg: TaskConfig | None) -> SurvivalConfig | None:
    ...
    if covariate_fusion not in {"feature_concat", "risk", "token_concat"}:
        raise ValueError("finetune.survival.covariate_fusion must be 'feature_concat', 'risk', or 'token_concat'.")
    if covariate_fusion in {"risk", "token_concat"} and not covariates:
        raise ValueError(
            f"finetune.survival.covariate_fusion='{covariate_fusion}' requires finetune.survival.covariates."
        )
