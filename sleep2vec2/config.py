...
class BackboneConfig:
    ...
    Notes:
      - sleep2vec2 uses a local, embeddings-only RoFormer encoder implementation
        (see :mod:`sleep2vec2.backbones.roformer_encoder`).
      - ``vocab_size`` is kept for backward compatibility with older YAML
        configs from the original sleep2vec recipe. It is ignored by the
        embeddings-only encoder.
    """

    name: str = "roformer"

    # --- Core RoFormer sizes ---
    # Optional input embedding size. If not set, defaults to hidden_size.
    embedding_size: int | None = None
    hidden_size: int = 768
    num_hidden_layers: int = 12
    num_attention_heads: int = 16
    # Optional FFN expansion. If not set, defaults to 4x hidden_size.
    ...
    # Catch-all overrides for advanced configs.
