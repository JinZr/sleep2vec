...
class KaldiChannelSpec:
    name: str
    input_dim: int
    scp_path: Path


class KaldiReaderPool:
    def __init__(self, root: str | Path, channel_specs: t.Mapping[str, KaldiChannelSpec]) -> None:
    ...
    def _normalize_specs(
        self,
        channel_specs: t.Mapping[str, KaldiChannelSpec],
        ...
        for channel, spec in channel_specs.items():
            ...
            specs[str(channel)] = KaldiChannelSpec(
                name=str(spec.name),
    ...
    def read_matrix(self, channel: str, key: str) -> np.ndarray:
        ...
        if arr.shape[1] != spec.input_dim:
            raise ValueError(
                f"Kaldi matrix for channel {channel!r}, key {key!r} has input_dim={arr.shape[1]}, "
                f"expected input_dim={spec.input_dim}."
            )
        return arr.copy()
