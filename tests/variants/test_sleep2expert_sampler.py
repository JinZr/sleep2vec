...
def test_sleep2expert_pair_eval_sampler_supports_lightning_distributed_rebuild() -> None:
    dataset = _Dataset()
...
def test_sleep2expert_pair_eval_sampler_exposes_lightning_sampler_argument() -> None:
    parameters = list(inspect.signature(SequentialPairEvalBatchSampler.__init__).parameters)
    assert parameters[1] == "sampler"


def test_sleep2expert_pair_eval_sampler_shards_pair_batches_evenly_after_lightning_rebuild() -> None:
