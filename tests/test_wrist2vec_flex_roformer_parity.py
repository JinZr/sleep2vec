...
from wrist2vec_flex.backbones.encoder_factory import build_roformer
...
def test_wrist2vec_flex_roformer_builder_uses_standalone_model():
...
def test_wrist2vec_flex_roformer_sdpa_matches_eager_with_padding_mask():
    config_kwargs = {
        "vocab_size": 37,
        "embedding_size": 16,
        "hidden_size": 16,
...
def test_wrist2vec_flex_roformer_sdpa_config_keeps_output_attentions():
...
def test_wrist2vec_flex_roformer_matches_hf_forward_with_copied_weights():
...
def test_wrist2vec_flex_pretrain_forward_uses_standalone_roformer():
    from wrist2vec_flex.config import (
        BackboneConfig as FlexBackboneConfig,
        ChannelConfig as FlexChannelConfig,
        ClsConfig as FlexClsConfig,
        ModelConfig as FlexModelConfig,
        ProjectionConfig as FlexProjectionConfig,
        TokenizerConfig as FlexTokenizerConfig,
    )
    ...
    flex_config = FlexModelConfig(
        channels=[
            FlexChannelConfig(
                name=name,
                input_dim=input_dim,
                tokenizer=FlexTokenizerConfig(name="linear", out_dim=16),
            )
            for name, input_dim in channel_specs
        ],
        backbone=FlexBackboneConfig(**backbone_kwargs),
        projection=FlexProjectionConfig(name="simclr", enabled=True, hidden_dim=32, out_dim=8),
        cls=FlexClsConfig(downstream="tokens", embedding_type=None),
    )

    torch.manual_seed(123)
    model = Wrist2vecPretrainModel(
        model_config=flex_config,
        device="cpu",
    ...
    assert isinstance(model.encoder, RoFormerEncoderModel)
    assert model.encoder.config.attention_backend == "sdpa"
    assert first_hidden.shape == (2, 5, 8)
    assert second_hidden.shape == (2, 5, 8)
