...
@pytest.mark.parametrize("package_name", ["sleep2vec", "sleep2vec2", "sleep2expert"])
def test_arousal_ddp_records_single_rank_returns_local_records_without_object_gather(
    ...
    assert finetuning_cls._gather_arousal_event_records(module, local_records) is local_records


@pytest.mark.parametrize("package_name", ["sleep2vec", "sleep2vec2", "sleep2expert"])
def test_arousal_ddp_records_use_all_gather_object_fallback(package_name: str, monkeypatch: pytest.MonkeyPatch) -> None:
    finetuning_mod = importlib.import_module(f"{package_name}.sleep2vec_finetuning")
    finetuning_cls = finetuning_mod.Sleep2vecFinetuning
    module = finetuning_cls.__new__(finetuning_cls)
    ...
    assert finetuning_cls._gather_arousal_event_records(module, local_records) == [{"rank": 0}, {"rank": 1}]


@pytest.mark.parametrize("package_name", ["sleep2vec", "sleep2vec2", "sleep2expert"])
@pytest.mark.parametrize("rank", [0, 1])
def test_arousal_ddp_records_fail_without_global_object_gather(
    package_name: str, rank: int, monkeypatch: pytest.MonkeyPatch
) -> None:
    finetuning_mod = importlib.import_module(f"{package_name}.sleep2vec_finetuning")
    finetuning_cls = finetuning_mod.Sleep2vecFinetuning
    ...
    with pytest.raises(RuntimeError, match="cannot gather arousal records"):
        finetuning_cls._gather_arousal_event_records(module, [{"rank": rank}])
