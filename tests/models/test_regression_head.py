...
def test_regression_head_num_layers(package_name: str, num_layers: int, expected_dims: list[tuple[int, int]]):
    module = importlib.import_module(f"{package_name}.downstreams.heads.regression")
    head = module.build_regression_head(
        target="age",
        feature_dim=4,
        n_mods=2,
        output_dim=2,
    ...
    linears = [layer for layer in head.regressor if isinstance(layer, nn.Linear)]
...
def test_regression_head_rejects_invalid_num_layers(package_name: str, num_layers):
    module = importlib.import_module(f"{package_name}.downstreams.heads.regression")

    with pytest.raises(ValueError, match="num_layers must be 1, 2, or 3"):
        module.RegressionHead(
            target="age",
            feature_dim=4,
            n_mods=2,
            out_dim=2,
