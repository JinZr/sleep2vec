...
from sleep2vec2.backbones.encoder_factory import build_roformer
...
def _copy_sleep2vec_forward_weights(base_model, sleep2vec2_model) -> None:
    base_state = base_model.state_dict()
    sleep2vec2_state = sleep2vec2_model.state_dict()
    missing: list[str] = []

    for name, target in sleep2vec2_state.items():
    ...
    if missing:
        raise AssertionError(f"sleep2vec2 forward state has no sleep2vec source for: {missing}")

    _copy_hf_weights_to_standalone(base_model.encoder, sleep2vec2_model.encoder)


def test_sleep2vec2_roformer_builder_uses_standalone_model():
...
def test_sleep2vec2_pretrain_forward_matches_sleep2vec_with_identical_config():
    ...
    from sleep2vec2.pretrain_model import Sleep2vecPretrainModel as VariantSleep2vecPretrainModel
