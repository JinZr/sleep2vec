...
`finetune_balanced` requires a complete explicit `finetune.layer_mix` mapping and an explicit `finetune.lora` control mapping containing both control booleans; disabled or single-channel LayerMix must not retain inert layer/share settings. Other omitted LoRA fields keep the canonical variant loader defaults, while the generated config, hashes, and runtime/repository identity preserve provenance. Without `inputs.pretrained_backbone_path`, LoRA adaptation remains fixed because hparam `ckpt_path` is final-evaluation-only. The profile reports only axes with more than one candidate as searched and does not claim that unknown config fields were inventoried.
