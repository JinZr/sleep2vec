...
If the tool returns `NEEDS_USER_INPUT`, stop and ask the user the generated questions. When `doctor --output-dir` or a safely published blocked `plan` emits `decisions.yaml`, fill only user-authorized values and pass that file back with `--user-decisions`. A blocked-plan retry must use a fresh output directory. A doctor retry may reuse its output directory only while the existing `decisions.yaml` already contains every currently requested decision; newly exposed decisions require a fresh doctor output directory. `context` remains diagnostic-only and does not emit this template. Do not run training. Do not generate executable scripts. Do not evaluate external test data.
