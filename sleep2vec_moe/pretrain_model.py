...
from .builders import build_encoder_factory, build_projection, build_tokenizers_and_dim
...
class Sleep2vecPretrainModel(nn.Module):
    ...
    def _token_embeddings_to_hidden(self, token_embeddings, batch, *, return_router: bool = False):
        ...
        enc_out = self._run_encoder(token_embeddings, first_padding_mask, return_router=return_router)

        if return_router:
            hidden, router_outputs = enc_out
            return hidden, router_outputs

        return enc_out

    def _run_encoder(self, token_embeddings, attention_mask, *, return_router: bool = False):
        """Routes embeddings through the selected encoder."""
        if self._custom_encoder_forward is not None:
            out = self._custom_encoder_forward(self.encoder, token_embeddings, attention_mask)
            if return_router:
                return out, None
            return out
    ...
    def forward(self, batch, apply_mask, return_router: bool = False):
        ...
        # 3/4/5
        if return_router:
            first_hidden, first_router = self._token_embeddings_to_hidden(
                first_mod_token_embeddings, batch, return_router=True
            )
            second_hidden, second_router = self._token_embeddings_to_hidden(
                second_mod_token_embeddings, batch, return_router=True
            )
        else:
        ...
        if return_router:
            return first_hidden, second_hidden, [first_router, second_router]
