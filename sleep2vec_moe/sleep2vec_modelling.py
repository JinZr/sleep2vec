...
class Sleep2vecPretraining(pl.LightningModule):
    def __init__(self, args, model_config, loss_config, averaging_config=None):
        ...
        self._last_moe_metrics: dict[str, torch.Tensor] = {}
    ...
    def _contrastive_step(self, batch, log_prefix=None, model=None):
        ...
        model_out = model(batch, apply_mask=True)
        if isinstance(model_out, (list, tuple)) and len(model_out) == 3:
            first_hidden, second_hidden, aux = model_out
        else:
            first_hidden, second_hidden = model_out
            aux = {"moe_loss": torch.zeros((), dtype=torch.float32, device=first_hidden.device)}

        loss_out = self.loss_fn(first_hidden, second_hidden, batch)
        moe_loss = aux.get("moe_loss")
        if moe_loss is None:
            moe_loss = torch.zeros((), dtype=torch.float32, device=first_hidden.device)
        total_loss = loss_out.loss + moe_loss
        ...
        if log_prefix is not None:
            ...
            scalar_log_map = {
                "moe/avg/dropped_fraction": "moe_dropped_frac",
                "moe/avg/load_cv2": "moe_load_cv2",
                "moe/avg/importance_cv2": "moe_importance_cv2",
                "moe/avg/lb_loss_raw": "moe_lb_loss_raw",
                "moe/avg/group_loss_raw": "moe_group_loss_raw",
                "moe/avg/z_loss_raw": "moe_z_loss_raw",
            }
            for metric_key, metric_name in scalar_log_map.items():
                metric_val = aux.get(metric_key)
                if metric_val is None:
                    continue
                if torch.is_tensor(metric_val) and metric_val.numel() == 1:
                    self.log(
