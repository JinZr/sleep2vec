...
from sleep2vec.callbacks.moe_logger import MoEUtilizationLoggerCallback
...
def sleep2vec_pretrain(args):
    ...
    logger = WandbLogger(
        project="sleep2vec-moe-pretrain",
        name=f"moe-pretrain-{run_name}",
    ...
    if bool((model_config.backbone.config_overrides or {}).get("moe_enabled", False)):
        callbacks.append(MoEUtilizationLoggerCallback(every_n_steps=args.moe_log_every_n_steps))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--moe-log-every-n-steps",
        type=int,
        default=200,
        help="Interval in optimizer steps for logging MoE expert utilization.",
    )
    parser.add_argument(
        "--strategy",
        type=str,
