...
- `runtime.device`, `runtime.num_workers`, `runtime.batch_size`, `runtime.limit_records`, and `runtime.dry_run`: optional `sleep2stat run` CLI knobs. `runtime.num_workers` also controls summarize sidecar reads when `summarize_after_run` is enabled.
