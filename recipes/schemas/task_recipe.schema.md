...
The supported task values are `preset_prepare`, `finetune`, `infer`, `evaluate`, `embedding_extraction`, `hparam_tune`, and `sleep2stat`. Embedding extraction supports `sleep2vec`, `sleep2vec2`, and `sleep2expert`; it requires an explicit NPZ index and rejects `data_backend`, preset/Kaldi inputs, dataset source overrides, `runtime.batch_size`, and `execution`. Other model tasks use `sleep2vec`, `sleep2vec2`, `sleep2expert`, or `sex_age_baseline` as applicable; `sleep2stat` omits `variant` or sets it to `null`.
