...
- `runtime`:
  - `finetune`: `accumulate_grad_batches`, `batch_size`, `check_val_every_n_epoch`, `ckpt_every_n_epochs`, `device`, `devices`, `epochs`, `gradient_clip_val`, `lr`, `lr_scheduler`, `lr_decay_floor`, `lr_decay_shape`, `lr_decay_ratio`, `lr_plateau_factor`, `lr_plateau_patience`, `num_workers`, `patience`, `precision`, `warmup_steps`, `wandb_mode`, `weight_decay`. `sex_age_baseline` omits `wandb_mode` and the six `lr_scheduler`/`lr_decay_*`/`lr_plateau_*` fields because its renderer does not consume them.
