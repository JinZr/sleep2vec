...
- New `preset_prepare` plans without authored Python/commit identity default to the current manager interpreter, Git HEAD, and `REPO_ROOT` only for local/default-local execution at the exact manager checkout without remote path context. Other workdirs or remote path context require a complete explicit local identity; preset plans do not provide SSH launching. Missing manager Git identity fails before workspace creation. Partial identities fail instead of receiving defaults. Historical registered plans without identity keep their original commands without rebinding.
