...
- Hparam `execution`: `conda_env`, `env`, `gpu_pool`, `gpus_per_run`, `host`, `max_concurrent`, `path_context`, `path_validation`, `target`, `wandb_group`, `wandb_project`, `workdir`. `env` has dynamic environment-variable names but may not duplicate the explicit W&B fields.
