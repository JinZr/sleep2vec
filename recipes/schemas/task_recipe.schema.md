...
- Non-hparam `execution`: `host`, `path_context`, `path_validation`, `python`, `runtime_commit`, `target`, `workdir`. The runtime identity fields let a managed infer plan bind an immutable target checkout; `experiment-run` requires all three.
