...
- Hparam `execution.scheduler` is optional in authored recipes and is materialized as `{type: direct}` in resolved plans. `type: slurm` requires `partition`, positive `cpus_per_task`, Slurm `memory` such as `64G`, and `walltime` in `HH:MM:SS` or `D-HH:MM:SS` form; `nice` and `nodelist` are optional. Slurm submits one frozen leaf job per run, requires `gpus_per_run` to be a positive YAML integer when authored, defaults it to one, and derives logical `runtime.devices` as `[0, ..., N-1]`. It rejects `gpu_pool`, `max_concurrent`, `conda_env`, hparam-overlay `runtime.devices`, `SLURM_*` or `CUDA_VISIBLE_DEVICES` entries in `execution.env`, unknown scheduler keys, and arbitrary sbatch arguments. `target` remains only the local/SSH control transport. Schema-v1 external-evaluation pipelines remain direct-only.
