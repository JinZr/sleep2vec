...
from wrist2vec_flex.backbones.encoder_factory import TransformerEncoderFactory
from wrist2vec_flex.builders import build_encoder_factory, build_projection, build_tokenizers_and_dim
from wrist2vec_flex.cls import build_cls_embedding
from wrist2vec_flex.config import ModelConfig, ProjectionConfig
from wrist2vec_flex.modules.channel_source_encoder import ChannelSourceEncoder
from wrist2vec_flex.modules.tokenizers import LinearTokenizer, SundialTokenizer


class Wrist2vecPretrainModel(nn.Module):
    def __init__(
        self,
        ...
        if model_config is not None:
            ...
            source_encoder_channels = list(model_config.channels)
        ...
        if source_encoder_channels is None:
            self.channel_source_encoder_mapping = nn.ModuleDict(
                {
                    channel_name: ChannelSourceEncoder.single_source(
                        feature_dim=channel_feature_dim,
            ...
            self.channel_source_encoder_mapping = nn.ModuleDict(
                {
                    ...
                    for channel in source_encoder_channels
                }
            )
    ...
    def _metadata_for_channel(self, metadata: t.Mapping[str, torch.Tensor] | None, channel_name: str):
        if metadata is None:
            return None
        return metadata.get(channel_name)

    def _reject_effective_channel_names(self, names: t.Iterable[str]) -> None:
        effective_names = sorted(str(name) for name in names if "::" in str(name))
        if effective_names:
            raise ValueError(
                "Source-aware pretraining expects logical channel keys, not effective source branches: "
                f"{effective_names}"
            )

    def _encode_channel_sources(
        self,
        channel_name: str,
        tokens: torch.Tensor,
        *,
        ...
        logical_channel_name: str | None = None,
    ) -> torch.Tensor:
        logical_name = logical_channel_name or channel_name
        if logical_name != channel_name:
            return self._encode_effective_channel_source(logical_name, tokens)

        encoder = self.channel_source_encoder_mapping[logical_name]

        return encoder(
            tokens,
            tokenizer=self.tokenizer_mapping[logical_name],
            source_mask=source_mask,
            source_ids=source_ids,
        )

    def _encode_effective_channel_source(self, logical_name: str, tokens: torch.Tensor) -> torch.Tensor:
        if tokens.dim() == 4:
            if tokens.shape[2] != 1:
                raise ValueError(
                    "Legacy effective source branches must contain exactly one source, "
                    f"got shape {tuple(tokens.shape)} for logical channel '{logical_name}'."
                )
            tokens = tokens.squeeze(2)
        return self.tokenizer_mapping[logical_name](tokens)

    def _candidate_channels_for_batch(
        self,
        tokens: t.Mapping[str, torch.Tensor],
        channel_mask: torch.Tensor | None,
    ) -> list[str]:
        self._reject_effective_channel_names(tokens.keys())
        available = [channel_name for channel_name in self.channel_names if channel_name in tokens]
        if channel_mask is None:
            return available

        mask = channel_mask.to(dtype=torch.bool)
        if mask.dim() != 2 or mask.shape[1] != len(self.channel_names):
            raise ValueError(f"channel_mask must have shape [B, {len(self.channel_names)}], got {tuple(mask.shape)}.")
        present_for_batch = mask.all(dim=0).tolist()
        return [
            channel_name
            for channel_name, is_present in zip(self.channel_names, present_for_batch)
            if is_present and channel_name in tokens
        ]

    def _resolve_pretrain_pair(
        self,
        tokens: t.Mapping[str, torch.Tensor],
        *,
        pair: t.Sequence[str] | None = None,
        channel_mask: torch.Tensor | None = None,
    ) -> list[str]:
        candidates = self._candidate_channels_for_batch(tokens, channel_mask)

        if pair is not None:
            if not isinstance(pair, (list, tuple)) or len(pair) != 2:
                raise ValueError(f"batch pair must contain exactly two logical channels, got {pair!r}.")
            chosen = [str(pair[0]), str(pair[1])]
            self._reject_effective_channel_names(chosen)
            unknown = [channel_name for channel_name in chosen if channel_name not in self.channel_names]
            if unknown:
                raise ValueError(f"batch pair contains unknown logical channels: {unknown}.")
            missing = [channel_name for channel_name in chosen if channel_name not in tokens]
            if missing:
                raise ValueError(f"batch pair channels are missing from tokens: {missing}.")
            if channel_mask is not None:
                unavailable = [channel_name for channel_name in chosen if channel_name not in candidates]
                if unavailable:
                    raise ValueError(f"batch pair channels are not available for every sample: {unavailable}.")
            return chosen

        specified = list(self.specified_two_mods or [])
        if len(specified) >= 2:
            specified = [str(specified[0]), str(specified[1])]
            self._reject_effective_channel_names(specified)
            if all(channel_name in candidates for channel_name in specified):
                return specified

        if len(candidates) < 2:
            raise ValueError(f"可用通道不足 2 个：{candidates}")
        return random.sample(candidates, 2)

    def _tokenize_two_random_channels(
        self,
        tokens,
        *,
        pair=None,
        ...
        chosen_channels = self._resolve_pretrain_pair(tokens, pair=pair, channel_mask=channel_mask)
        return {
            channel_name: self._encode_channel_sources(
                channel_name,
                tokens[channel_name],
                source_mask=self._metadata_for_channel(source_mask, channel_name),
                source_ids=self._metadata_for_channel(source_ids, channel_name),
            )
            for channel_name in chosen_channels
        }

    def _tokenize_one_channel(self, tokens, *, source_mask=None, source_ids=None):
    ...
    def _tokenize_all(
        ...
        *,
        ...
        source_mask: t.Mapping[str, torch.Tensor] | None = None,
        source_ids: t.Mapping[str, torch.Tensor] | None = None,
    ):
        """
        对每个模态做tokenization
        """
        ...
        return {
            channel_name: self._encode_channel_sources(
                ...
                source_mask=self._metadata_for_channel(source_mask, channel_name),
                source_ids=self._metadata_for_channel(source_ids, channel_name),
                logical_channel_name=logical_lookup.get(channel_name, channel_name),
            )
    ...
    def forward(self, batch, apply_mask):
        ...
        token_embeddings = self._tokenize_two_random_channels(
            tokens,
            pair=batch.get("pair"),
    ...
    def encode(self, batch, channel_name):
        self._reject_effective_channel_names([channel_name])
        tokens = batch["tokens"]
        if channel_name not in tokens:
            raise ValueError(f"Channel '{channel_name}' is missing from batch tokens.")

        # 1. tokenize
        token_embeddings = self._encode_channel_sources(
            channel_name,
            tokens[channel_name],
            source_mask=self._metadata_for_channel(batch.get("source_mask"), channel_name),
            source_ids=self._metadata_for_channel(batch.get("source_ids"), channel_name),
        )
    ...
    def set_tokenizers_trainable(self, trainable: bool) -> None:
        """Freeze/unfreeze learned input-representation parameters without altering module mode."""
            ...
            for name, module in self.named_children():
                if "tokenizer" in name:
                    for param in module.parameters():
                        param.requires_grad = trainable
        if hasattr(self, "channel_source_encoder_mapping"):
            for param in self.channel_source_encoder_mapping.parameters():
                param.requires_grad = trainable
    ...
    def freeze_backbone_groups(
        ...
        if train_tokenizers:
            _select_parameters(
                lambda n: n.startswith(("tokenizer_mapping.", "channel_source_encoder_mapping."))
                or any(key in n for key in ["high_tokenizer", "low_tokenizer"])
            )

        # 4) 冻结了 tokenizer 时，把其中的 BN/Dropout 置 eval，避免统计量漂移
        if not train_tokenizers:
            for module in self._tokenizer_modules(self.channel_names):
                module.eval()
    ...
    def _adaptation_group_for_param(self, name: str, new_channels: t.Set[str]) -> str | None:
        ...
        if name.startswith("channel_source_encoder_mapping."):
            channel_name = name.split(".", 2)[1]
            return "new_modalities" if channel_name in new_channels else "legacy_modalities"
    ...
    def _tokenizer_modules(self, channel_names: t.Iterable[str]) -> list[nn.Module]:
        ...
        for channel_name in channel_names:
            ...
            if hasattr(self, "channel_source_encoder_mapping") and channel_name in self.channel_source_encoder_mapping:
                modules.append(self.channel_source_encoder_mapping[channel_name])
        return modules
