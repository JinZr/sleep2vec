...
from wrist2vec_flex.common import VARIABLE_CHANNEL_KALDI_ERROR, is_builtin_seq_task, validate_kaldi_source_contract
from wrist2vec_flex.data.kaldi_psg_dataset import KaldiPSGDataset
from wrist2vec_flex.data.metadata import _encode_binary_label, safe_cast
from wrist2vec_flex.data.psg_pretrain_dataset import PSGPretrainDataset
from wrist2vec_flex.data.samplers import SequentialPairEvalBatchSampler
...
def _build_finetune_loader(
    ...
    *,
    ...
    data_backend = getattr(args, "data_backend", "npz")
    if data_backend == "kaldi" and bool(getattr(args, "allow_missing_feature_channels", False)):
    ...
    feature_channel_names = list(args.data_channel_names)
    ...
    if is_seq_task:
        ...
        required_runtime_channel_names.append(label_source_name)
    for auxiliary_name in getattr(args, "auxiliary_label_source_names", []) or []:
        if auxiliary_name not in required_runtime_channel_names:
            required_runtime_channel_names.append(auxiliary_name)
    dataset_channel_names = list(feature_channel_names)
    for channel_name in required_runtime_channel_names:
        if channel_name not in dataset_channel_names:
            dataset_channel_names.append(channel_name)
    ...
    dataset_kwargs = dict(
        ...
        allow_missing_channels=bool(getattr(args, "allow_missing_feature_channels", False)),
        min_channels=int(getattr(args, "min_feature_channels", len(feature_channel_names))),
    ...
    if data_backend == "npz":
        dataset_kwargs.update(
            ...
            expand_source_branches=False,
            required_channel_names=required_runtime_channel_names,
            collate_all_feature_channels=True,
        )
    else:
        dataset_kwargs.update(
            kaldi_data_root=args.kaldi_data_root,
