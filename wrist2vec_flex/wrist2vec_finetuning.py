...
from wrist2vec_flex import diagnostics
from wrist2vec_flex.averagings.base import BaseModelAverager, build_model_averager
from wrist2vec_flex.checkpoints import is_source_module_state_key
from wrist2vec_flex.common import remap_stage_labels
from wrist2vec_flex.distributed import get_rank_world_size, is_torch_distributed_ready
from wrist2vec_flex.metrics import (
...
from wrist2vec_flex.visualization.downstream_eval import DownstreamEvalVisualizer
from wrist2vec_flex.visualization.layer_mix import build_layer_mix_rows, render_layer_mix_heatmap

from .downstream_model import Wrist2vecDownstreamModel
from .pretrain_model import Wrist2vecPretrainModel


class Wrist2vecFinetuning(pl.LightningModule):
    def __init__(self, args, model_config, finetune_config=None, averaging_config=None):
        ...
        self.backbone = Wrist2vecPretrainModel(
            ...
            device=args.device,
            source_dropout_rate=getattr(args, "source_dropout_rate", 0.0),
            min_sources_after_dropout=getattr(args, "min_sources_after_dropout", 1),
        ).to(args.device)

        head_kwargs = getattr(args, "head_kwargs", None)
        self.model = Wrist2vecDownstreamModel(
            args.label_name,
            self.backbone,
            channel_names=list(args.data_channel_names),
            ...
            channel_dropout_rate=getattr(args, "channel_dropout_rate", 0.0),
            min_channels_after_dropout=getattr(args, "min_channels_after_dropout", 1),
        ).to(args.device)

        if args.pretrained_backbone_path:
            logging.info(f"loading pretrain model from {args.pretrained_backbone_path}")
            load_info = self.model.load_pretrained_backbone(args.pretrained_backbone_path)
            source_missing = [key for key in load_info.missing_keys if is_source_module_state_key(key)]
            if source_missing and getattr(args, "freeze_tokenizer", True):
                raise ValueError(
                    "wrist2vec_flex source modules are missing from the warm-start checkpoint while "
                    "freeze_tokenizer=True. Use a wrist2vec_flex checkpoint or set freeze_tokenizer=False."
                )
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        ...
        source_missing = [k for k in result.missing_keys if is_source_module_state_key(k)]
        if source_missing and getattr(self.args, "freeze_tokenizer", True):
            raise ValueError(
                "wrist2vec_flex source modules are missing from the checkpoint while "
                "freeze_tokenizer=True. Use a wrist2vec_flex checkpoint or set freeze_tokenizer=False."
            )
        if not strict:
            return result
        ...
        missing = [
            k for k in result.missing_keys if not k.startswith(allowed_prefixes) and not is_source_module_state_key(k)
        ]
        ...
        layer_mix_missing = [k for k in result.missing_keys if k.startswith(allowed_prefixes)]
        if source_missing:
            logging.warning(
                "Missing source module keys while loading checkpoint; source fusion/embedding modules are "
                "initialized from the current config: %s",
                source_missing,
            )
        if layer_mix_missing:
            logging.warning("Missing layer-mix keys while loading checkpoint: %s", layer_mix_missing)
