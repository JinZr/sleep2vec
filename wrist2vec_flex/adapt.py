...
def _resolve_adapt_run_artifacts(
    *,
    ...
    return AdaptRunArtifacts(
        save_path=Path("log-wrist2vec-flex-adapt") / run_name / "checkpoints",
        run_name=run_name,
        wandb_id=None,
        trainer_ckpt_path=None,
...
def wrist2vec_adapt(args):
    ...
    logger = WandbLogger(
        project="wrist2vec-flex-adapt",
        name=f"w2v-flex-adapt-{run_name}",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run staged modality adaptation for wrist2vec_flex.")
    ...
    parser.add_argument(
        ...
        choices=["npz", "kaldi"],
        default=None,
        help="Data backend for adaptation (default: npz unless YAML data.backend overrides it).",
    )
    parser.add_argument(
        "--kaldi-data-root",
        type=Path,
    ...
    wrist2vec_adapt(parser.parse_args())
