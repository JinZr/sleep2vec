...
from wrist2vec_flex.data.utils import default_extractor, default_mlm_mask_generator, default_tokenizer, window
...
class PSGPretrainDataset(DefaultDataset):
    def __init__(
        ...
        *,
        ...
        collate_all_feature_channels: bool = False,
        ...
        **kwargs: t.Any,
        ...
        self.feature_channel_names = list(feature_channel_names or channel_names)
        unknown_feature_channels = sorted(set(self.feature_channel_names) - set(channel_names))
        ...
        self.required_channel_names = list(required_channel_names or [])
        unknown_required_channels = sorted(set(self.required_channel_names) - set(channel_names))
        if unknown_required_channels:
            raise ValueError(f"required_channel_names must be a subset of channel_names: {unknown_required_channels}")
        self.collate_all_feature_channels = bool(collate_all_feature_channels)
        self.channel_input_dims = _normalize_channel_input_dims(channel_input_dims)
        self.channel_source_names = normalize_channel_source_names(channel_names, channel_source_names)
        self.expand_source_branches = bool(expand_source_branches)
        (
            self.output_channel_names,
            self.effective_channel_to_logical,
            self.effective_channel_to_source,
        ) = build_effective_channel_mappings(channel_names, self.channel_source_names)
