...
def _is_sample_index_like(obj: t.Any) -> bool:
    """Duck-type check that accepts SampleIndex from any module version.

    Presets may be generated by ``wrist2vec.preprocess.save_dataset_presets``
    (which stores ``wrist2vec.data.default_dataset.SampleIndex``) and later
    consumed by ``wrist2vec_flex``.  A strict ``isinstance`` check would fail
    across module boundaries, so we match on structural attributes instead.
    """
    return isinstance(obj, SampleIndex) or (
        hasattr(obj, "id") and hasattr(obj, "path") and hasattr(obj, "start") and hasattr(obj, "end")
    )
    ...
    def dataloader(self, device: str = "cpu") -> DataLoader:
        ...
        def collate_fn(indices, tolerance=1):
            ...
            for item in indices:
                src = item
                pair = None
                if isinstance(item, tuple) and len(item) == 2 and _is_sample_index_like(item[0]):
                    src, pair = item
                if not _is_sample_index_like(src):
