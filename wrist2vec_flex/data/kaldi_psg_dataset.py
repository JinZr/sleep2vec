...
from wrist2vec_flex.data.default_dataset import DefaultDataset, SampleIndex
from wrist2vec_flex.data.kaldi_io import KaldiChannelSpec, KaldiReaderPool
from wrist2vec_flex.data.psg_pretrain_dataset import _build_channel_registry, _normalize_channel_input_dims
...
def _validate_single_source_kaldi_contract(
    channel_names: t.Sequence[str],
    channel_source_names: t.Mapping[str, t.Sequence[str]] | None,
) -> None:
    normalized = normalize_channel_source_names(channel_names, channel_source_names)
    unsupported = {
        channel: sources
        for channel, sources in normalized.items()
...
class KaldiPSGDataset(DefaultDataset):
    def __init__(
        ...
        **dataloader_kwargs: t.Any,
        ...
        _validate_single_source_kaldi_contract(self.channel_names, channel_source_names)
        self.channel_source_names = normalize_channel_source_names(self.channel_names, channel_source_names)
        self.expand_source_branches = False
        self.feature_channel_names = list(feature_channel_names or self.channel_names)
        unknown_feature_channels = sorted(set(self.feature_channel_names) - set(self.channel_names))
        if unknown_feature_channels:
            raise ValueError(f"feature_channel_names must be a subset of channel_names: {unknown_feature_channels}")
    ...
    def _load_tokens_for_src(
        ...
        **_: t.Any,
        ...
        for channel in chosen_output_names:
        ...
        masks = {channel: self.mask_generators[channel](tokens[channel]) for channel in chosen_output_names}
        source_masks = {channel: torch.ones(1, dtype=torch.bool) for channel in chosen_output_names}
        source_ids = {channel: torch.arange(1, dtype=torch.long) for channel in chosen_output_names}
        return {}, tokens, masks, dict(src.metadata), source_masks, source_ids
