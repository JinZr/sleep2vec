...
from wrist2vec_flex.source_routing import (
...
def load_channel_source_tokens(
    npz,
    *,
    channel_name: str,
    start: int,
    ...
    channel_input_dims: t.Mapping[str, int] | None,
    source_names: t.Sequence[str],
    available_source_names: t.Sequence[str],
    dtype: torch.dtype = torch.float32,
    tolerance: int = 1,
) -> tuple[dict[str, torch.Tensor], torch.Tensor, torch.Tensor, torch.Tensor]:
    configured_sources = [str(source_name) for source_name in source_names] or [str(channel_name)]
    available_sources = {str(source_name) for source_name in available_source_names}
    if not available_sources and configured_sources == [str(channel_name)]:
        available_sources = {str(channel_name)}

    payload_by_source: dict[str, torch.Tensor] = {}
    tokens_by_source: dict[str, torch.Tensor] = {}
    source_mask = torch.zeros(len(configured_sources), dtype=torch.bool)

    for source_idx, source_name in enumerate(configured_sources):
        if source_name not in available_sources:
            continue
        payload, tokens = load_channel_tokens(
            npz,
            channel_name=channel_name,
            start=start,
            end=end,
        ...
        payload_by_source[source_name] = payload
        tokens_by_source[source_name] = tokens
        source_mask[source_idx] = True

    if not tokens_by_source:
        raise ValueError(f"Channel '{channel_name}' is marked available but has no usable configured sources.")

    lengths = [tokens.shape[0] for tokens in tokens_by_source.values()]
    max_len, min_len = max(lengths), min(lengths)
    if max_len - min_len > tolerance:
        raise ValueError(f"Source token length mismatch > tolerance for channel '{channel_name}': {lengths}")

    first_tokens = next(iter(tokens_by_source.values()))[:min_len]
    token_shape = first_tokens.shape[1:]
    token_dtype = first_tokens.dtype
    for source_name, tokens in tokens_by_source.items():
        if tokens.shape[1:] != token_shape:
            raise ValueError(
                f"Source token shape mismatch for channel '{channel_name}' source '{source_name}': "
                f"expected (*, {tuple(token_shape)}), got {tuple(tokens.shape)}."
            )
        tokens_by_source[source_name] = tokens[:min_len]

    ordered_tokens = []
    for source_name in configured_sources:
        source_tokens = tokens_by_source.get(source_name)
        if source_tokens is None:
            source_tokens = torch.zeros((min_len, *token_shape), dtype=token_dtype)
        ordered_tokens.append(source_tokens)

    if len(ordered_tokens) == 1:
        stacked_tokens = ordered_tokens[0]
    else:
        stacked_tokens = torch.stack(ordered_tokens, dim=1)
    source_ids = torch.arange(len(configured_sources), dtype=torch.long)
    return payload_by_source, stacked_tokens, source_mask, source_ids


def make_missing_channel_tokens(
    channel_name: str,
    length: int,
    channel_input_dims: t.Mapping[str, int] | None,
    source_names: t.Sequence[str],
    ...
    configured_sources = [str(source_name) for source_name in source_names] or [str(channel_name)]
    frames_per_token = _frames_per_token_for_channel(channel_name, channel_input_dims)
    token_shape = (int(length), frames_per_token)
    if len(configured_sources) > 1:
        token_shape = (int(length), len(configured_sources), frames_per_token)
    tokens = torch.zeros(token_shape, dtype=torch.float32)
    source_mask = torch.zeros(len(configured_sources), dtype=torch.bool)
    source_mask[0] = True
    source_ids = torch.arange(len(configured_sources), dtype=torch.long)
    return tokens, source_mask, source_ids
...
def filter_valid_sample_indices(
    ...
    *,
    ...
    feature_channel_names: t.Sequence[str] | None = None,
    required_channel_names: t.Sequence[str] | None = None,
    ...
    - allow_missing_channels=True: keep samples with >= min_channels available feature channels
    ...
    required_channel_names = list(dict.fromkeys(str(name) for name in (required_channel_names or [])))
    if feature_channel_names is None:
        label_channel_names = {"stage5", "ahi"}
        feature_channel_names = [
            name for name in channel_names if name not in label_channel_names and name not in required_channel_names
        ]
    else:
        feature_channel_names = list(dict.fromkeys(str(name) for name in feature_channel_names))
    requires_builtin_ahi = "ahi" in extractors and (
        not allow_missing_channels or "ahi" in feature_channel_names or "ahi" in required_channel_names
    )
                ...
                for sample_index in samples:
                    try:
                        ...
                        if allow_missing_channels:
                            ...
                            available_features = [name for name in feature_channel_names if name in available_set]
                            missing_required = [name for name in required_channel_names if name not in available_set]
                            if missing_required:
                                logging.info(
                                    "[Skip] Missing required channels at %s: missing=%s. Meta: %s",
                                    sample_index.id,
                                    missing_required,
                                    getattr(sample_index, "metadata", {}),
                                )
                                continue
                            if len(available_features) < min_channels:
                                logging.info(
                                    "[Skip] Not enough feature channels at %s: have=%d need>=%d. Meta: %s",
                                    sample_index.id,
                            ...
                            payload = {}
                            tokens = {}
                            load_names = list(available_features)
                            for key in required_channel_names:
                                if key not in load_names:
                                    load_names.append(key)
                            for key in load_names:
                                if not expand_source_branches and normalized_source_names[key] != [key]:
                                    payload[key], tokens[key], _, _ = load_channel_source_tokens(
                                        npz,
                                        channel_name=key,
                                        start=sample_index.start,
                                        end=sample_index.end,
                                        channel_input_dims=channel_input_dims,
                                        source_names=normalized_source_names[key],
                                        available_source_names=channel_sources.get(key, []),
                                        tolerance=tolerance,
                                    )
                                else:
                            ...
                            for key in channel_names:
                                ...
                                if explicit_source_names:
                                    payload[key], tokens[key], _, _ = load_channel_source_tokens(
                                        ...
                                        available_source_names=matched_sources,
                                        tolerance=tolerance,
                                    )
                                else:
                                    source_name = choose_source_name(key, channel_sources=channel_sources)
                        ...
                        if max_len - min_len <= tolerance:
                            ...
                            if isinstance(payload_dict, dict):
                                if allow_missing_channels:
                                    payload_dict["available_channels"] = list(available_features)
