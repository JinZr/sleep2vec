...
_LABEL_CHANNEL_NAMES = {"stage5", "ahi"}
_SOURCE_FUSION_NAMES = {"identity", "masked_mean", "masked_gated_scalar", "learned_query"}
_SOURCE_FUSION_KWARGS = {
    "identity": set(),
    "masked_mean": set(),
    "masked_gated_scalar": set(),
    "learned_query": {"dropout"},
}
_CHANNEL_AGG_NAMES = {"mean", "masked_mean", "concat", "gated_scalar", "masked_gated_scalar", "learned_query"}
...
@dataclass
class SourceFusionConfig:
    name: str = "identity"
    kwargs: dict[str, t.Any] = field(default_factory=dict)


@dataclass
class SourceEmbeddingConfig:
    enabled: bool = False
...
class ChannelConfig:
    ...
    source_fusion: SourceFusionConfig | None = None
    source_embedding: SourceEmbeddingConfig | None = None

    def __post_init__(self) -> None:
        if self.source_fusion is None:
            self.source_fusion = SourceFusionConfig(name="learned_query" if len(self.source_names) > 1 else "identity")
        if self.source_embedding is None:
            self.source_embedding = SourceEmbeddingConfig(enabled=len(self.source_names) > 1)
...
class FinetuneDataConfig:
    ...
    allow_missing_feature_channels: bool = False
    min_feature_channels: int | None = None
    ...
    min_channels_after_dropout: int = 1
    source_dropout_rate: float = 0.0
    min_sources_after_dropout: int = 1

    def __post_init__(self) -> None:
        self.backend = _validate_data_backend(self.backend)
        if not isinstance(self.allow_missing_feature_channels, bool):
            raise ValueError("data.allow_missing_feature_channels must be a boolean.")
        if self.min_feature_channels is not None and (
            not isinstance(self.min_feature_channels, int)
            or isinstance(self.min_feature_channels, bool)
            or self.min_feature_channels <= 0
        ):
            raise ValueError("data.min_feature_channels must be a positive integer when provided.")
        if (
            not isinstance(self.channel_dropout_rate, (int, float))
            or isinstance(self.channel_dropout_rate, bool)
            or not 0.0 <= float(self.channel_dropout_rate) <= 1.0
        ):
            raise ValueError("data.channel_dropout_rate must be in [0.0, 1.0].")
        self.channel_dropout_rate = float(self.channel_dropout_rate)
        if (
            not isinstance(self.min_channels_after_dropout, int)
            or isinstance(self.min_channels_after_dropout, bool)
            or self.min_channels_after_dropout <= 0
        ):
            raise ValueError("data.min_channels_after_dropout must be a positive integer.")
        if (
            not isinstance(self.source_dropout_rate, (int, float))
            or isinstance(self.source_dropout_rate, bool)
            or not 0.0 <= float(self.source_dropout_rate) <= 1.0
        ):
            raise ValueError("data.source_dropout_rate must be in [0.0, 1.0].")
        self.source_dropout_rate = float(self.source_dropout_rate)
        if (
            not isinstance(self.min_sources_after_dropout, int)
            or isinstance(self.min_sources_after_dropout, bool)
            or self.min_sources_after_dropout <= 0
        ):
            raise ValueError("data.min_sources_after_dropout must be a positive integer.")
...
class PretrainDataConfig:
    ...
    channel_dropout_rate: float = 0.0
    min_sources_after_dropout: int = 1
    min_channels_after_dropout: int = 2
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError("data.channel_dropout_rate must be in [0.0, 1.0].")
        if float(self.channel_dropout_rate) != 0.0:
            raise ValueError("data.channel_dropout_rate is not supported for wrist2vec_flex pretrain.")
        self.channel_dropout_rate = 0.0
        if (
            not isinstance(self.min_sources_after_dropout, int)
            or isinstance(self.min_sources_after_dropout, bool)
            or self.min_sources_after_dropout <= 0
        ...
        if (
            ...
            raise ValueError("data.min_channels_after_dropout must be a positive integer.")
        if self.min_channels_after_dropout != 2:
            raise ValueError(
                "data.min_channels_after_dropout must remain 2 because pretrain channel dropout is unsupported."
            )


def _build_source_fusion_config(raw: t.Any, *, channel_name: str, source_count: int) -> SourceFusionConfig:
    if raw is None:
        return SourceFusionConfig(name="learned_query" if source_count > 1 else "identity")
    if not isinstance(raw, dict):
        raise ValueError(f"channel '{channel_name}' source_fusion must be a mapping.")

    extra = sorted(set(raw.keys()) - {"name", "kwargs"})
    if extra:
        raise ValueError(f"channel '{channel_name}' source_fusion has unsupported fields: {extra}")

    name = raw.get("name", "learned_query" if source_count > 1 else "identity")
    if not isinstance(name, str) or not name:
        raise ValueError(f"channel '{channel_name}' source_fusion.name must be a non-empty string.")

    kwargs = raw.get("kwargs", {})
    if kwargs is None:
        kwargs = {}
    if not isinstance(kwargs, dict):
        raise ValueError(f"channel '{channel_name}' source_fusion.kwargs must be a mapping.")
    _validate_source_fusion_kwargs(channel_name, name, kwargs)

    return SourceFusionConfig(name=name, kwargs=dict(kwargs))


def _validate_source_fusion_kwargs(channel_name: str, fusion_name: str, kwargs: t.Mapping[str, t.Any]) -> None:
    if fusion_name not in _SOURCE_FUSION_NAMES:
        return
    unsupported = sorted(set(kwargs) - _SOURCE_FUSION_KWARGS[fusion_name])
    if unsupported:
        raise ValueError(
            f"channel '{channel_name}' source_fusion.kwargs has unsupported fields "
            f"for '{fusion_name}': {unsupported}"
        )


def _build_source_embedding_config(raw: t.Any, *, channel_name: str, source_count: int) -> SourceEmbeddingConfig:
    if raw is None:
        return SourceEmbeddingConfig(enabled=source_count > 1)
    if not isinstance(raw, dict):
        raise ValueError(f"channel '{channel_name}' source_embedding must be a mapping.")

    extra = sorted(set(raw.keys()) - {"enabled"})
    if extra:
        raise ValueError(f"channel '{channel_name}' source_embedding has unsupported fields: {extra}")

    enabled = raw.get("enabled", source_count > 1)
    if not isinstance(enabled, bool):
        raise ValueError(f"channel '{channel_name}' source_embedding.enabled must be a boolean.")

    return SourceEmbeddingConfig(enabled=enabled)
...
def _require_channels(model_block: dict[str, t.Any]) -> t.List[ChannelConfig]:
    ...
    for item in channels_raw:
        ...
        if not tok_name:
            raise ValueError(f"channel '{channel_name}' must set tokenizer.name.")
        ...
        if tok_cfg.out_dim is None:
            raise ValueError(f"channel '{channel_name}' must set tokenizer.out_dim.")

        raw_source_names = item.pop("source_names", None)
        raw_source_fusion = item.pop("source_fusion", None)
        raw_source_embedding = item.pop("source_embedding", None)

        if channel_name in _LABEL_CHANNEL_NAMES:
            if raw_source_names is not None:
                raise ValueError(f"channel '{channel_name}' cannot set source_names.")
            if raw_source_fusion is not None:
                raise ValueError(f"channel '{channel_name}' cannot set source_fusion.")
            if raw_source_embedding is not None:
                raise ValueError(f"channel '{channel_name}' cannot set source_embedding.")
        ...
        source_fusion_cfg = _build_source_fusion_config(
            raw_source_fusion,
            channel_name=channel_name,
            source_count=len(source_names),
        )
        source_embedding_cfg = _build_source_embedding_config(
            raw_source_embedding,
            channel_name=channel_name,
            source_count=len(source_names),
        )
        ...
        channels.append(
            ChannelConfig(
                ...
                source_fusion=source_fusion_cfg,
                source_embedding=source_embedding_cfg,
                **item,
            )
        )
    return channels
...
def validate_model_config(model_cfg: ModelConfig) -> int:
    ...
    if model_cfg.head is not None:
        ...
        if model_cfg.head.channel_agg.name not in _CHANNEL_AGG_NAMES:
            raise ValueError(f"model.head.channel_agg.name must be one of {sorted(_CHANNEL_AGG_NAMES)}.")

    for channel in model_cfg.channels:
        if len(set(channel.source_names)) != len(channel.source_names):
            raise ValueError(f"channel '{channel.name}' source_names must not contain duplicates.")

        source_fusion = channel.source_fusion
        if source_fusion is None:
            source_fusion = SourceFusionConfig(name="learned_query" if len(channel.source_names) > 1 else "identity")
            channel.source_fusion = source_fusion
        if source_fusion.name not in _SOURCE_FUSION_NAMES:
            raise ValueError(
                f"channel '{channel.name}' source_fusion.name must be one of {sorted(_SOURCE_FUSION_NAMES)}."
            )
        if not isinstance(source_fusion.kwargs, dict):
            raise ValueError(f"channel '{channel.name}' source_fusion.kwargs must be a mapping.")
        _validate_source_fusion_kwargs(channel.name, source_fusion.name, source_fusion.kwargs)

        source_embedding = channel.source_embedding
        if source_embedding is None:
            source_embedding = SourceEmbeddingConfig(enabled=len(channel.source_names) > 1)
            channel.source_embedding = source_embedding
        if not isinstance(source_embedding.enabled, bool):
            raise ValueError(f"channel '{channel.name}' source_embedding.enabled must be a boolean.")

        if channel.name in _LABEL_CHANNEL_NAMES and channel.source_names:
            raise ValueError(f"channel '{channel.name}' cannot set source_names.")
        if channel.name in _LABEL_CHANNEL_NAMES and (source_fusion.name != "identity" or bool(source_fusion.kwargs)):
            raise ValueError(f"channel '{channel.name}' cannot set source_fusion.")
        if channel.name in _LABEL_CHANNEL_NAMES and source_embedding.enabled:
            raise ValueError(f"channel '{channel.name}' cannot set source_embedding.")
        if len(channel.source_names) > 1 and source_fusion.name == "identity":
            raise ValueError(
                f"channel '{channel.name}' source_fusion.name='identity' cannot be used with multiple source_names."
            )
