...
from wrist2vec_flex.callbacks import build_distributed_ahi_progress_bar
from wrist2vec_flex.common import apply_finetune_config, persist_run_config_and_args
from wrist2vec_flex.results import save_result_csv
from wrist2vec_flex.utils import get_finetune_dataloaders
...
def supervised(args, config_bundle):
    ...
    exp_root = Path(f"log-wrist2vec-flex-finetune/{args.version}/")
    ...
    logger = WandbLogger(
        project="wrist2vec-flex-finetune",  # 相当于 TensorBoard 的 log dir
        name=f"w2v-flex-finetune-{version}",  # run 名称
    ...
    try:
        ...
        checkpoint_callback = ModelCheckpoint(
            dirpath=f"log-wrist2vec-flex-finetune/{version}/checkpoints",  # ← 你想要的目录
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fine-tune wrist2vec_flex downstream models on PSG data.")
