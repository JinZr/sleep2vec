...
from wrist2vec_flex.callbacks.pair_acc_logger import PairAccLoggerCallback
from wrist2vec_flex.checkpoints import is_source_module_state_key, load_pretrain_init_weights
from wrist2vec_flex.common import apply_data_backend_args, apply_model_config_args, persist_run_config_and_args
...
def wrist2vec_pretrain(args):
    ...
    args.source_dropout_rate = config_bundle.data.source_dropout_rate
    args.min_sources_after_dropout = config_bundle.data.min_sources_after_dropout
    ...
    if args.ckpt_path is not None and os.path.isfile(args.ckpt_path):  # NEW
        ...
        save_path = f"log-wrist2vec-flex-pretrain/{run_name}/checkpoints"
    ...
    model = Wrist2vecPretraining(args, model_config, loss_config, averaging_config=averaging_config)
    if args.pretrained_backbone_path and args.ckpt_path is None:
        ...
        if source_missing:
            logging.warning(
                "Missing source module init keys (%d); source fusion/embedding modules are initialized "
                "from the current config: %s",
                len(source_missing),
                source_missing,
            )
        if missing_keys:
            logging.warning("Missing init keys: %s", missing_keys)
    ...
    logger = WandbLogger(
        project="wrist2vec-flex-pretrain",
        name=f"w2v-flex-pretrain-{run_name}",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Extra tag inserted into log-wrist2vec-flex-pretrain/<run_name>; useful for noting "
