...
from wrist2vec_flex.config import AdaptConfig
from wrist2vec_flex.data.channel_selection import build_all_pairs
...
class Wrist2vecAdaptation(Wrist2vecPretraining):
    def __init__(self, args, model_config, loss_config, adapt_config: AdaptConfig, averaging_config=None):
        ...
        if args.pretrained_backbone_path and args.ckpt_path is None:
            ...
            if source_missing:
                logging.warning(
                    "Missing source module adaptation init keys (%d); source fusion/embedding modules are "
                    "initialized from the current config: %s",
                    len(source_missing),
                    source_missing,
                )
            if missing_keys:
                logging.warning("Missing adaptation init keys: %s", missing_keys)
