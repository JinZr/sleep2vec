...
from wrist2vec_flex.checkpoints import is_source_module_state_key, load_checkpoint, load_pretrain_init_weights
from wrist2vec_flex.config import HeadConfig, LayerMixConfig, ModelConfig
from wrist2vec_flex.modules.layer_mix import LayerMix
...
from .pretrain_model import Wrist2vecPretrainModel
...
class Wrist2vecDownstreamModel(nn.Module):
    def __init__(
        self,
        target: str,
        ...
        self.channel_names = (
            list(effective_channel_names) if effective_channel_names is not None else list(channel_names)
        )
        logical_channel_set = set(self.logical_channel_names)
        unknown_active_channels = [
            channel_name
            for channel_name in self.channel_names
            if self.effective_channel_to_logical.get(channel_name, channel_name) not in logical_channel_set
        ]
        if unknown_active_channels:
            raise ValueError(
                "Downstream active channels must map to model.channels. "
                f"Unknown active channels: {unknown_active_channels}; model channels: {self.logical_channel_names}."
            )
        self.device = device
        ...
        self.is_seq = is_seq
        self.target = target
        if not 0.0 <= float(channel_dropout_rate) <= 1.0:
            raise ValueError("channel_dropout_rate must be in [0.0, 1.0].")
        if int(min_channels_after_dropout) <= 0:
            raise ValueError("min_channels_after_dropout must be > 0.")
        self.channel_dropout_rate = float(channel_dropout_rate)
        self.min_channels_after_dropout = int(min_channels_after_dropout)
        ...
        head_kwargs.setdefault("agg_kwargs", dict(channel_cfg.kwargs or {}))
    ...
    def _feature_channel_mask(
        self,
        channel_mask: torch.Tensor | None,
        token_names: t.Sequence[str],
    ) -> torch.Tensor | None:
        ...
        if mask.dim() != 2:
            raise ValueError(f"channel_mask must be rank-2, got shape {tuple(mask.shape)}.")

        token_names = list(token_names)
        if mask.shape[1] == len(token_names):
            return mask

        if mask.shape[1] == len(self.logical_channel_names):
            logical_to_idx = {name: idx for idx, name in enumerate(self.logical_channel_names)}
            columns = []
            for token_name in token_names:
                logical_name = self.effective_channel_to_logical.get(token_name, token_name)
                if logical_name not in logical_to_idx:
                    raise ValueError(f"channel_mask has no logical column for channel '{logical_name}'.")
                columns.append(mask[:, logical_to_idx[logical_name]])
            return torch.stack(columns, dim=1)

        raise ValueError(
            "channel_mask width must match either downstream feature count "
            f"({len(token_names)}) or logical channel count ({len(self.logical_channel_names)}); "
            f"got {mask.shape[1]}."
        )
    ...
    def _apply_channel_dropout(
        ...
        if not self.training or self.channel_dropout_rate <= 0.0:
            return channel_mask

        if channel_mask is None:
            first_tokens = tokens[token_names[0]]
        ...
        return apply_mask_dropout(
            ...
            self.min_channels_after_dropout,
        )

    @staticmethod
    def _source_mask_for_tokenization(
        source_mask: t.Mapping[str, torch.Tensor] | None,
        channel_mask: torch.Tensor | None,
        token_names: t.Sequence[str],
    ) -> t.Mapping[str, torch.Tensor] | None:
        if source_mask is None or channel_mask is None:
            return source_mask

        patched = dict(source_mask)
        for mod_idx, token_name in enumerate(token_names):
            mask = source_mask.get(token_name)
            if mask is None:
                continue
            present = channel_mask[:, mod_idx].to(device=mask.device, dtype=torch.bool)
            if present.all():
                continue
            updated = mask.clone()
            absent = ~present
            if updated.dim() == 2:
                ...
                updated[absent, 0] = True
            elif updated.dim() == 3:
                updated[absent] = False
                updated[absent, :, 0] = True
            patched[token_name] = updated
        return patched

    def forward(self, batch):
        tokens = batch["tokens"]
        requested_channel_names = list(self.channel_names)
        feature_channel_mask = self._feature_channel_mask(batch.get("channel_mask"), requested_channel_names)
        feature_channel_mask = self._apply_channel_dropout(feature_channel_mask, tokens, requested_channel_names)
        source_mask = self._source_mask_for_tokenization(
            batch.get("source_mask"),
            feature_channel_mask,
            requested_channel_names,
        )

        token_embeddings = self.backbone._tokenize_all(
            tokens,
            channel_names=requested_channel_names,
        ...
        if self.is_seq and token_masks:
            merged_mask = None
            for mod_idx, mask in enumerate(token_masks):
                if mask is None:
                    continue
                if feature_channel_mask is not None:
                    present = feature_channel_mask[:, mod_idx].to(device=mask.device, dtype=torch.bool)
                    mask = mask & present.unsqueeze(1)
            ...
            output = self._call_head(feature_of_different_mods, merged_mask, feature_channel_mask)
        else:
            output = self._call_head(feature_of_different_mods, None, feature_channel_mask)
        return output
    ...
    def load_pretrained_backbone(self, ckpt_path, use_ema: bool | str | None = True):
        ...
        source_missing = [key for key in load_info.missing_keys if is_source_module_state_key(key)]
        missing_keys = [key for key in load_info.missing_keys if not is_source_module_state_key(key)]
        unexpected_keys = load_info.unexpected_keys

        logging.info(f"✅ Loaded {total_keys - len(load_info.missing_keys)} / {total_keys} keys into backbone.")
        if source_missing:
            logging.warning(
                "Missing source module keys (%d); source fusion/embedding modules are initialized "
                "from the current config:",
                len(source_missing),
            )
            for k in source_missing:
                logging.warning(f"    {k}")
        if missing_keys:
